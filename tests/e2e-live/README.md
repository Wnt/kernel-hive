...
| `anon-clean-load.mjs` | Does a STRANGER's first load come back clean, and does the page-load beacon fire? Counts failed requests, console errors and `/eum` beacons on the live landing page. The beacon count is expected to be **0 for a signed-out visitor**: `/eum` is deliberately not in `OPEN_PATHS` (see `eum_proxy.py` — an anonymous ingest is a free write into the tenant), so the telemetry half only passes for a session that may report. `node anon-clean-load.mjs` |
| `walkin-url-probe.mjs` | Does a walk-in visitor KEEP their machine across a reload? `/` claims whichever station the broker has, so the landing page publishes it into the URL (`/walkin/<station>`) and that address re-claims the same one. Drives the deployed bundle as a real anonymous visitor and checks three things: `/` publishes a station, a reload keeps it, and the link pins it for a cold visitor. **Claims real pool cells for the length of the run — do not loop it.** `node walkin-url-probe.mjs` |
