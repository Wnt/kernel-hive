...
dbus-rel 7, warpd 5, gallery-hid 1, mamesock 3** (`irix`, `w2kalpha`, `tru64`),
**mgactl 1** (`aix432`), **ramabs 1** (`macos753`).
...
| **ramabs** (absolute write) | `macos753` | abs | absolute `MOVEA x y` over a chardev QEMU itself serves (`-global nubus-macfb.ptrctl=`), but there is NO cursor register and no loop: the engine writes the target straight into the guest OS's own in-RAM pointer state — on macos753 the Mac low-memory globals `MTemp` ($0828) and `RawMouse` ($082C), published with `CrsrNew` ($08CE) := `CrsrCouple` ($08CF) — and Mac OS's own cursor VBL task moves the pointer and states where it landed in `Mouse` ($0830), which is the read-back the ack waits on | The sensor class for a guest that has NO hardware cursor (the OS composites the sprite in software). Nothing converges, nothing is calibrated, and the hotspot never enters the control path because `Mouse` holds the POINTER. Costs one guest-specific control object per OS — address, field order and endianness all differ — and the addresses are only valid against the golden they were probed on. **Never write the read-back global**: on Mac OS, pre-writing `Mouse` defeats the VBL task's change detector and the cursor silently does not move |
