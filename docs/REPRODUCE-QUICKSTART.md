...
- The SPA installs and builds locally without private credentials.
...
Use the SPA path on a normal development machine. Use the full-box path only
when rebuilding the intended x86-64 Proxmox environment; run host-side phases
as root and workstation-side phases where the master runbook says to. The host
...
internet access, and the lab's host layout; a successful local `--dry-run` does
not prove those prerequisites.

## 2. Build or run the SPA locally
...
It therefore needs neither the lab box nor guest credentials. Live tile
streaming still requires the HTTPS serve plane and streamhost instances from the
full-box path.

## 3. Prepare for a full box rebuild
...
3. [`../streamhost/bring-up-all.sh`](../streamhost/bring-up-all.sh) together
   with the per-tile deploy section of
   ...
   its local CA or deploying the SPA bundle.
...
On the target host, inspect the build plan and preflight before building:
...
checks only the selected/default build set; on a fresh host that default set
...
- `docs/gallery-credentials.md` and `spa/src/data/credentials.ts` for private
  guest-login hints. Neither is required for the placeholder SPA build.
...
documented host locations, emits the tile files, and runs
...
it on a replacement host until there is an intentional live reference to
compare.
...
- Real credentials, tokens, SSH private keys, guest-login tables, and serving
  PKI. The SPA carries only a placeholder credential example.
- Dependency and build output: `node_modules/`, SPA `dist/`, Rust `target/`,
  caches, logs, and test artifacts.
- The built `/data/gallery-guests/` images, `/data/vms/streamhost/tiles/` live
  state, golden snapshots, runtime cert hashes, service state, and the deployed
  SPA webroot.
...
- The optional prebuilt-image migration source. MASTER-REPRODUCE Phase 4′ is a
  shortcut that depends on the old dry-run box, not part of from-source
  reproducibility.
...
- A fully curated fleet still has golden-fixture gaps. MASTER-REPRODUCE's
  "Gaps / still-manual" section identifies tile-local golden disk copies and
  bake helpers that are not yet fully reconstructed from tracked files.
  Android, FreeDOS, and 9front need a first launch without their unconditional
  `-loadvm golden` before a new golden can be baked.
...
- `scripts/dev/verify-emit.sh` proves parity with one existing lab inventory;
  it does not prove that a brand-new box has complete guest artifacts.
