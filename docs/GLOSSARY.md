...
| **wave** | One or more sessions integrating new stations in parallel, sharing one landing lock and allocating from one pool (`scripts/dev/wave.sh alloc`); a wave's own session and worktree are still per-station (`<id>`, `<id>-<stream>`), same as any other task under rule 3. | integration wave, station wave (informally, unchanged) |
| **landing window** | The serialized span in which one wave (and only one) pushes to `main` and deploys: fetch+merge main → validate/generate → push → `box-deploy --apply` → golden swap → `station-up` → proofs → SPA deploy. Held as a `kh-claim` resource (`wave.sh land begin/end/status`) so only one wave holds it at a time; `station-land.sh` runs the whole window as one command. | "the window", the "go" → "landed" span (informally) |
| **plane** | One channel of connectivity a station can join, verified independently: the **retronet web plane** (a station's tap reaches the corpus proxy) and the **ICQ/IM plane** (an account is signed in and reachable). "On the plane" means `rn-verify.sh` passes for it — host ping is never sufficient (containment). | — |
