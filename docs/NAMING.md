...
React UI, the docs, the museum concept, the GitHub repo. Use "Kernel Hive"
...
- **`streamhost`** — the Rust daemon that captures a station's display and audio
  ...
  the live labhost: renaming it would mean touching every unit file, every
...
- **`labctl`** — the operator CLI on labhost (`scripts/labctl`, installed at
  `/usr/local/bin/labctl` on labhost) used to drive stations: `labctl ls`,
  ...
  byte-identical to their live copies on labhost, so they are never
  rebranded or otherwise edited casually.

A few other identifiers also stay as they are for the same "it's a live
labhost resource, not branding" reason: the `osgallery-dev` hostname for the
...
that file is the map of what is actually deployed on labhost today.
...
or a script labhost already has installed under a fixed name — it keeps its
existing name.
