# labhost harvest and drift gate

The two directions are intentionally separate. `build-deploy.sh` mirrors repo
source to labhost; `harvest.sh` is the reviewable labhost-to-repo path for the small
set of files that may be authored or reconciled live.

## Harvest labhost changes
...
manifests, tracked verbatim station launchers, `labctl`, the live `tiles.json`
reference, seven declared `serve/` mirrors, and the labhost checkout's registry.
...
UTC stamp, and aggregate MD5 of the labhost source tree. Deploy tooling uses the
...
This is read-only on labhost. It prints one row per concrete mirror with the
repo MD5, labhost MD5, and `MATCH`/`DRIFT`, then exits non-zero if any row drifts.
The registry uses the union of allowed files so a file present on only one side
cannot disappear from the report. Generated (non-verbatim) station launchers remain
