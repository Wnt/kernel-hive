# IRIX station — measurement baseline, 2026-08-03
...
against an assumption that labhost was quiet.

- `labhost`, 16 logical CPUs, load average 0.59 / 0.68 / 1.91 at 18:56 EEST.
- **Zero station services running.** All 31 `streamhost@*` units `inactive (dead)`,
...
The live station directory showed a launch at 18:35 and files last written 18:44,
i.e. the station had been run and stopped earlier that day. It was **not** running.
...
| `irix65-apps-v3.chd` (**shipped seed**) | `368fcfb9b56fb4165a4e456238dc1a18` |
...
All seven seeds are `chmod 444` and carry the `i` (immutable) attribute.
`tiles/irix/disk.chd` is **not** a seed — it is the per-launch writable copy
...
seed with the production binary and production flags, drives the login, and
...
Same binary, same seed, same `-video none` + shm publish, same `-sound none`,
same `-frameskip 6`, same `irixagent.lua`. The rig adds **`-nothrottle`**.

The station itself runs throttled (`x11-runtime.sh` passes no `-nothrottle`), which
...
The brief asked for four workloads. Two of them cannot be driven on the seed
...
- **W1 terminal scroll** (`find /usr -print` in a winterm) and **W2 window drag
  during that scroll** need a shell inside the guest. Seed **v3 has no guest
...
That is the inversion in miniature, on labhost, in this cohort: do not read a
...
n=5 and its interval is wide. The foreign load was not a sibling agent — labhost
...
- The **only sub-100% regime measured is cold boot** (~94%). Everything a
  ...
  is already **above 100%**, where the station's throttle binds. A win there is
