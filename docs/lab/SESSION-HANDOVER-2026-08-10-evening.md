...
## 1. Live box state — read this before touching anything

**Five tiles are deliberately stopped.** None is broken:

| Tile | Why | Restart when |
|---|---|---|
| `indyr4400`, `star`, `nextstep` | The box's three biggest CPU consumers; the operator stopped them to free capacity | The measurement campaign ends |
...
**15 of 28 bridge tiles on trixie (54%)**, up from 3 this morning. `bookworm 13 ·
...
half a core per tile, with streamhost at a third of its bridged cost. Rounds
...
| `scripts/dev/box-sync-push.sh` | The repo→box half. Refuses box-authoritative rows, re-verifies through the gate's own reverse-scrub path |
...
**`migrate-tile.sh` had four bugs, all fixed.** Three cost live tiles: a poll
...
separate agents each rediscovered them, and each **damaged a live tile** —
`decos`, `plus4`, `bbcmicro` all had a golden re-baked over them by an abandoned
...
read-modify-write** of the shared ledger, so under `-j 2` two tiles finishing
together silently reverted one tile's migration while both reported success.
...
- **Architecture docs**: [`GUEST-TIERS.md`](../GUEST-TIERS.md) (five tiers, not
  three — `openvms` runs two sibling VMs, and the posters have no runtime),
...
- **Soft hide / dark launch**: `listing: { state, reason, since }` in a registry
  ...
  control. It hides a tile that **belongs** in the lineup; it cannot admit a
  non-lineup rig, because `gen_tiles_json.py` hard-fails on a declared tile with
  no live directory.
- **box-sync 205 → 232 pairs** — the serving plane and the whole auth plane had
  none. Found a `test-clientlog.sh` on the box that was **95 lines stale**.
- **Hourly ZFS snapshot of `data/vms`**, 7-day TTL, box-only, not in the repo —
...
- **`SPA id == SH_TILE`** — exactly two tiles diverge (`aros`/`amigaos`,
  `solaris`/`solariscde`). Renaming the **daemon** side to match the registry id,
  `aros` first. `solaris` is the riskiest tile in the fleet (patched QEMU with
...
2. Run the latency curve once the pointer works, or pivot the fixtures to
   ...
   quadrature.
3. Un-hide and restart `atarist`; restart the three stopped tiles.
