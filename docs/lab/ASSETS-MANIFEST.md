...
| Dwarf/Draco Mesa emulator `dist.zip` | sha256 `67f84b77cbed6cba9d7d2485e84b8142e4fd2403243f8abd8f6e5a81ff6fcf75` *(pin, measured-match in-kiosk)* | 509 198 | `daybreak.sh` | daybreak kiosk guest `/opt/bridge/media/daybreak/dist.zip` — **PRESENT in-guest** (unpacked into the tile overlay + its golden; re-fetched by the builder) | `github.com/devhawala/dwarf` master; **BSD-3-Clause, redistributable** — but the licence covers the emulator ONLY, not the Xerox disk below |
| Xerox ViewPoint 2.0.5 Pilot disk `vp2.0.5.zdisk` | sha256 `02bdb53ba7f7896a914fe43b7ca19a620907d0fdbf0f55317b7d1f39aab3f872` *(pin, measured-match in-kiosk)* | 4 657 062 | `daybreak.sh` | daybreak kiosk guest `/opt/bridge/media/daybreak/vp2.0.5.zdisk` — **PRESENT in-guest** | `github.com/devhawala/dwarf` `disks-6085/`; **preservation-source** — Xerox-copyright ViewPoint/Pilot software rebuilt from the Bitsavers XDE 5.0 6085 disk plus the VP 2.0.5 floppy set. RUN privately here, streamed as pixels, never re-distributed and never given a download affordance. Its Software Options are unlocked but **bound to processor id `10-00-FE-31-AB-21`** — changing the emulated MAC re-locks every application |
