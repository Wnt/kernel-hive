# Retronet wave 2026-08-23 — four ICQ onboardings, macOS on the bridge, and
# retronet/ICQ made visible in the fleet table

**Status: RUNNING.** Six parallel streams. This file is the wave's **allocation
ledger and coordination contract** — read it before touching anything shared.
It is deliberately short: each stream's detail lives in its own station doc.

Parents: [`RETRONET-BRIEF.md`](../RETRONET-BRIEF.md),
[`ICQ-ONBOARDING-PLAN.md`](ICQ-ONBOARDING-PLAN.md). The ICQ recipe to follow is
[`ICQ-STATION-win2000.md`](ICQ-STATION-win2000.md) (**ICQ 2001b / SSI** — the
fleet standard; do not re-derive it, and do not onboard anything new on 2000b).

## The streams

| Stream | Owner | What "done" means |
|---|---|---|
| **fleet-data** | schema + emitter + SPA | retronet membership and ICQ persona are first-class registry/roster data, rendered as a fleet-table column, cross-validated by `stations-registry.py` |
| **icq-win95** | `win95` | UIN `95000` signed in on **ICQ 2001b** (retires the 2000b `connect()` blocker recorded in [`ICQ-STATION-win95.md`](ICQ-STATION-win95.md)) |
| **icq-winxp** | `winxp` | UIN `51000` signed in |
| **icq-w2kalpha** | `w2kalpha` | UIN `50010` signed in — **x86 ICQ 2001b under NT/Alpha FX!32**, or a documented alternative |
| **icq-os2warp** | `os2warp` | UIN `23000` signed in on a native OS/2 OSCAR client |
| **rn-macos753** | `macos753` | on `vmbr-rn` at `10.99.0.23`, browsing the corpus; ICQ is **out of scope** for this wave |

## Allocation ledger — claim from here, never invent

Anything below is **already reserved for this wave**. Nothing else on
`vmbr-rn` may take these values. `10.99.0.22` belongs to the concurrent
**rhapsody** stream — leave it alone.

| Station | UIN | retronet IP | MAC (scheme `52:54:00:52:4e:<octet>`) | Note |
|---|---|---|---|---|
| win95 | `95000` | `10.99.0.13` (existing) | `…:0d` (existing) | already bridged; ICQ only |
| winxp | `51000` | `10.99.0.18` (existing) | `…:12` (existing) | already bridged; ICQ only |
| os2warp | `23000` | `10.99.0.19` (existing) | `…:13` (existing) | already bridged; ICQ only |
| w2kalpha | `50010` | `10.99.0.17` (existing) | `…:11` (existing) | already bridged; ICQ only |
| macos753 | `75300` (reserved, not this wave) | **`10.99.0.23` (NEW)** | **`08:00:07:00:00:17` (NEW** — Apple OUI is forced by the guest**)** | NIC add → device-set change → **cold** golden rebuild |

UIN scheme, for the record: the number encodes the OS generation
(`nt4`→`40000`, `winxp` = NT 5.1 →`51000`, `w2kalpha` = NT 5.0 on Alpha →`50010`,
`os2warp` = OS/2 v3 →`23000`). Rows for all five are **already committed** to
[`../../../scripts/retronet/icq/roster.json`](../../../scripts/retronet/icq/roster.json)
with `onboarded: false`.

Passwords go in gitignored `registry/local.env` as
`RETRONET_ICQ_<STATION>_PASS`; the account is created with
`rn-tool.py user-set <uin> <pass>` and opened with `rn-tool.py user-open <uin>`.

## Coordination contract

1. **Own stack, own worktree.** `scripts/dev/wt.sh new <stream>`. Land on `main`
   yourself; never force-push, never touch another stream's files.
2. **Claim before you touch a shared thing** — `kh-claim` for the station, its
   tap, its guard chain and any VMID. `ssh lab 'labctl who'` answers "whose is
   this?". Fail loudly on a taken claim; do not fall back.
3. **Work a clone, not the live station** (`wt.sh new` gives you the sandbox).
   Where a step genuinely has to touch the live golden, **back the golden up
   first** and say so in your report.
4. **Files only one stream may edit:**
   - `registry/stations/<your-station>.json` — yours alone.
   - `scripts/retronet/icq/roster.json` — flip **only your own row's**
     `onboarded` to `true`, on the last commit of your stream. One line each,
     so the merges are clean.
   - `scripts/stations_registry/fleet_table.py`, `spa/src/ui/fleetColumns.tsx`,
     `spa/src/data/fleetTable.ts`, `scripts/stations-registry.py` — **fleet-data
     only**. If you need a registry field that does not exist yet, ask
     fleet-data for it; do not add it yourself.
   - This file — append to §Log, nothing else.
5. **The SSI cross-list is a wave-end step, run ONCE.** Do not run
   `seed_contacts.py ssi --apply` yourself. Flip your roster row, report, and
   the coordinator runs the seeder across the whole fleet when the wave closes —
   otherwise every station's roster is re-seeded five times and the goldens
   disagree about who exists.
6. **`box-deploy.sh --apply` is the coordinator's call while this wave runs.**
   It reverts other streams' in-flight live edits. Push, then say you need it.
7. **Teardown is part of done** — released claims, and the check that proved it.

## Log

Append one line per stream milestone: `YYYY-MM-DDThh:mmZ <stream> <what>`.
- 2026-08-23 fleet-data: `retronet` registry block + ICQ-roster merge landed — 12 members backfilled, `Retronet` column on /fleet, `stations-registry.py` now fails the gate on address/plane/roster/doc drift.
- 2026-08-23T22:50Z icq-winxp: UIN `51000` LIVE on **ICQ 2001b build 3659** (fleet standard, no deviation). Silent self-reconnect at t+17s on the `labctl reset` wake, SSI roster by name, HiveBot greeted. Windows Firewall stays ON — the first-run alert is answered **Keep Blocking** (XP's firewall is inbound-only, so only ICQ's unused P2P listener is blocked). Two findings worth stealing: deliver the installer by **swapping the CD medium** (IE8's download window never paints and winxp has no exec channel), and **an idle-paused guest reacts to nothing, which reads as a wedge** — wake and verify through `labctl` / `scripts/dev/qmp-type.py` ([`../INPUT-DEBUGGING.md`](../INPUT-DEBUGGING.md)), and never restart mid-install. `51000:winxp` appended to `RN_BOT_PERSONAS` in `/etc/retronet/bot.env`; roster row flipped. Docs: [`ICQ-STATION-winxp.md`](ICQ-STATION-winxp.md).

2026-08-23T19:49Z coordinator box-deploy --apply → box at main@abb8171 (unblocks rhapsody launcher; no restarts)
2026-08-23T20:0xZ icq-winxp LIVE — UIN 51000, ICQ 2001b b3659, SSI, greeted
2026-08-23T2x:xxZ icq-os2warp LIVE — UIN 23000, ICQ/2 1.503i, legacy v5 UDP, client-local roster
2026-08-23T2x:xxZ coordinator bot.py GREETINGS/STATION_BLURB row for os2warp (w2kalpha row owed)
2026-08-23T23:5xZ icq-w2kalpha-b: transplant PROVEN — stock x86 ICQ 2001b installed on a throwaway x86 W2K clone and moved onto the Alpha (payload + exact registry delta); UIN 50010 signs into the gateway from w2kalpha, twice, the second time unattended after a power cycle on the saved password. NOT onboarded: Keep-connection-alive lives in ICQ's account DB and needs the Preferences UI, which is blocked by this station's pointer defect (MOVEA does not track; measured, and it is NOT mouse acceleration). Golden bake + reset acceptance still owed. Docs: ICQ-STATION-w2kalpha.md
2026-08-24T01:1xZ icq-w2kalpha LIVE — UIN 50010, x86 ICQ 2001b b3659 under FX!32 on Alpha. Keep-connection-alive is the `UseFirewallSessionTimeout` record in `2001a\<UIN>.dat`, set on an x86 config bed and carried across on the account FILES (portable both ways when the install path matches). Literal server host VERIFIED by tcpdump — SYN straight to 10.99.0.2:5190, no login.icq.com query. Reset acceptance done after the gateway DROPPED the session: reconnect +15 s, fresh ports 1031/1032, silent on the framebuffer. OWED at wave end: SSI seeding (roster empty) and a `retronet-bot` restart (running env lacks 50010:w2kalpha). Pointer was NOT a station defect — `move_abs` drops MOVEA during the per-connection corner-home while acking OK, so one-shot `labctl mctl` loses every move; held-open clients are pixel-exact. Docs: ICQ-STATION-w2kalpha.md
2026-08-24T05:40Z icq-win95 PARTIAL — UIN 95000 signs in on ICQ 2002a b3728 and the SSI roster renders BY NAME (HiveBot online; beos/nt4/solaris/tru64/win2000/win98se offline); HiveBot greeted. The IE4 requirement is settled: this box is OSR2.5 (SubVersionNumber " C"), its own OEM C:\WIN95\ ships the full IE 4.01 CAB set, and IE 4.01 Browser Only (no Desktop Update) is now installed from it — that is what made the owner-drawn contact list paint. Registry `os` corrected OSR2 → OSR2.5; IE start page repointed to the archived home.microsoft.com. NOT onboarded: "Keep connection alive" ships OFF and ICQ 2002a's Simple-Mode Main menu will not open under any input path, so Preferences is unreachable — gateway drops 95000 ~140s into an idle pause and `labctl reset` leaves a zombie socket for 10+ min. planes stays ["web"], roster row stays onboarded:false. OPERATOR DECISION: find a route to Advanced Mode/Preferences, or authorise a fourth `*-icq-nudge` (win95) alongside win98se/nt4/win2000. Docs: [`ICQ-STATION-win95.md`](ICQ-STATION-win95.md).

2026-08-24T12:0xZ icq-win95-d NOT ONBOARDED, blocker re-diagnosed and the nudge option RETIRED ON EVIDENCE. `Keep connection alive` is reachable after all — not via Preferences but via the registration wizard's **Connection Settings** button, which only appears on the *"Can't establish connection"* failure screen; provoke it by parking the pointer on `Next` while the link is up, `set_link pcnet.0 off`, clicking with QMP alone (warpnet rides the guest NIC, so it dies with the link), waiting ~110 s, then `set_link on`. Flag ticked, `UseFirewallSessionTimeout=1` verified in `95000.dat`, golden re-baked with it. It does not help: after the gateway DROPPED 95000 and `labctl reset win95`, tcpdump shows the guest probing its stale socket and the gateway answering **RST twice within 3 s** — and ICQ 2002a ignores it (no reconnect, panel still *Online*, roster intact; guest pings the gateway 0% loss throughout). Since a `*-icq-nudge` exists only to deliver that RST, **a win95 nudge would not work either** — the §Log's earlier OPERATOR DECISION is answered. Also: ICQ cold-boots healthy once `95000*.*` is cleared **including the `tmp` pair** — that pair is why the predecessor's account-file transplant hung on "Loading...". Added MSTCP `KeepAliveTime=60000` (Win95 defaults to 2 h, which is why an earlier 7-min watch saw zero packets). Station recovered from having **no golden snapshot at all** (a predecessor was killed between `delvm golden` and its `savevm`). planes stays ["web"], roster row stays false. Docs: [`ICQ-STATION-win95.md`](ICQ-STATION-win95.md).
2026-08-24T14:5xZ icq-win95-e (win95-icq2000b) LIVE AND ONBOARDED — the operator's original ask ("make 2000b work") is answered: the historical "2000b never dials out, zero packets" was NOT reproducible; a clean ICQ 2000b b3281 install (empty ICQ dir, prereqs already on the stack) dials out, signs UIN 95000 in, and — with Keep connection alive ON — self-heals across `labctl reset` with NO nudge. Mechanism measured on the wire: keep-alive ON = a 6-byte FLAP ping every ~58 s; after a reset the first ping hits the dead socket, the gateway RSTs, and 2000b re-dials silently (+42 s on the production acceptance run; 2002a provably ignores that same RST, which is why the nudge was rightly retired for it and is unnecessary for 2000b). TRAP that almost shipped: 2000b flushes Server-tab prefs + saved password to the DB only on a graceful ICQ exit — a cold boot after an ungraceful stop reverts host/keep-alive/password (first golden shipped keep-alive OFF this way; caught by the failed first acceptance, wire-diagnosed, re-set, recaptured). Both goldens taken via `checkpoint-guard recapture` (dirty-probe wrinkle: focus the panel's search box first). Client swapped 2002a→2000b on the live disk from the proven clone; 2002a rollback RETAINED at golden-backup-ie401-icq2002a-keepalive-20260824 (bc4490d8…). planes ["web","icq"], roster row icq2000b/onboarded:true — SSI cross-list seeding + box-deploy --apply owed by coordinator. Docs: [`ICQ-STATION-win95.md`](ICQ-STATION-win95.md).
