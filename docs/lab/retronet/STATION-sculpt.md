# sculpt on the retronet — web (as built)
...
| | |
|---|---|
| retronet address | `10.99.0.38`, **dhcp** |
| MAC | fleet scheme `52:54:00:52:4e:<last IP octet>`. Real value box-local in gitignored `registry/local.env` as `RN_SCULPT_MAC`; the committed launcher and `rn-tapnet.sh` carry the scrubbed placeholder `02:00:00:00:00:26` and read the one line at boot. **It lives in the golden's device vmstate, so changing it needs a cold re-bake** |
| tap / bridge | `sculptrn0` on `vmbr-rn`, persistent, created + guarded by `streamhost/stations/sculpt/rn-tapnet.sh up` from the launcher on every start |
| guard chain | `SCULPTRN-IN` — ESTABLISHED,RELATED RETURN, everything the guest opens toward labhost DROP, hooked into `INPUT` on both the guest IP and the guest MAC |
| ICQ | UIN `19000`, nickname `sculpt`, password in `registry/local.env` as `RETRONET_ICQ_SCULPT_PASS` |
| joined | 2026-09-09 |
...
| | |
|---|---|
| NIC (retronet) | TODO — `-netdev tap,id=rn0,ifname=sculptrn0,script=no,downscript=no -device <model>,netdev=rn0,mac="$RN_SCULPT_MAC"`. Say which model and why (NE2000 is 16-bit PIO and costs a VM exit per word under KVM; `rtl8139` is the fleet's proven DMA-capable choice; two IDENTICAL cards make the guest's interface numbering a coin toss a `loadvm` cannot re-litigate) |
...
- Server-side: `rn-tool.py user-set 19000 <pass>` + `user-open 19000` +
  `nick 19000 sculpt` + `ssi-seed 19000 10000=HiveBot` (all done by
  `rn-onboard.sh --apply`).
...
| connect `10.99.0.1:8443` (the gallery) | **BLOCKED** by `SCULPTRN-IN` | TODO |
...
1. `rn-verify.sh sculpt` on the box (tap UP on vmbr-rn, unit active, the tap named
