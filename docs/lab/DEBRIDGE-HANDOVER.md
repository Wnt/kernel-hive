...
| Audio | `-sound sdl -audiodriver disk` → FIFO | `-sounddev fifo -soundarg <fifo> -soundrate 48000 -soundoutput 2` → FIFO (the fork's own header-free sink; `wav` until 2026-08-17, still the launcher's fallback for an older binary). **Never the sdl+disk shape** — VICE's `sdl` device is a timing source and the pipe becomes the clock (24 % speed) |
...
**Note for c128:** the VDC's snapshot module does not go through
`raster_snapshot_read()`, so on a VDC canvas the hold would expire on its bound
instead of releasing on the restore.
...
- **Only plus4, cbm2, pet2001 and cbm8032 run the `d518f3dbc5` binaries.**
  vic20, c128 and c64 are on older builds and keep `-sounddev wav` through the
  launcher's own fallback until they are rebuilt: rebuilding them is what gives
  them the click fix and the clean restore cut.
