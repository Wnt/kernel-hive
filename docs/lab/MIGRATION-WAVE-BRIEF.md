# Wave brief — read this before you run a kiosk migration

You are migrating kiosks from the bookworm guest base to trixie. **13 of
...
On 2026-08-10 three agents ran waves in parallel and migrated 12 stations. Each of
the three independently rediscovered the *same* three driver bugs, and each one
damaged a live production station before diagnosing it. This brief exists so that
...
|---|---|---|
| `scripts/dev/migrate-wave.sh` (+ `migrate-wave-plan.py`, `registry/bridge-waves.json`) | Hand-rolled concurrency, load checks, chroot serialization and the summary table | `migrate-wave.sh --wave 3 --dry-run` first, always. Caps stations in flight (`-j`, default 2), re-checks labhost load before every launch (`--max-load`, default 10.0 of 16 threads), takes an atomic `mkdir` claim on the shared trixie MAME chroot, refuses stopped stations and `c64` by name **before anything starts**, and prints the per-station verdict table + `--json` |
...
- **`amiga`'s headline blocker is already solved.** `bridge-base.sh` installs
  ...
  line is a no-op (`command -v fs-uae` short-circuits). The station is mechanical.
...
- `MIGRATE_BAKE_SETTLE` (default 180 s) is one global number applied to exactly
  the stations whose readiness no builder can check. A too-short settle captures a
  mid-load frame as the permanent scene every visitor resets to. Watch the
...
- **Never claim visual acceptance from a log, an exit code, or an assertion.**
  A station can be healthy in all three and render black (`amiga`/Mesa) or show a
...
- **Teardown is part of done, and is stated in the report.** A detached builder
  that outlived its driver re-captured a checkpoint into a live overlay three times in
  one day. Kill by pidfile, verify through `/proc/<pid>/exe`, release every
  claim (chroot claim, displays, taps), and say so.

---

## 4. When a station fails: roll back, diagnose, MOVE ON

**Six stations with one clean failure beats one station fixed and five unattempted.**
...
5. **Go to the next station.** Do not open a second investigation inside a wave.

A station you never attempted is not a failure. A station you half-migrated and left
undeclared is.
...
2. **Per-station verdict table** — station · MIGRATED / ROLLED-BACK / REFUSED /
...
Two waves flipping *different* stations on *adjacent* alphabetical lines is a git
conflict, and "take ours" / "take theirs" silently reverts a whole wave's work.
Merge key by key: a station changed on one side takes that side's value; a station
...
lines in the script whose bugs had already damaged three live stations. Wave agents
