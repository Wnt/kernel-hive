...
scripts/dev/smoke-rig.sh <id> --like <sibling>            # claims slot/port/vmid, stream.env, daemon, dark-launch
                                                           # x11/shm guest: placeholder qmp.sock + patch stream.env (wall table below)
...
| `cursor-locate.py learn` returns `AMBIGUOUS` at every position over a uniform background | the guest's pointer is XOR/inverting, not a blitted sprite (breaks the exact-match premise) | `cursor-locate-cv.py` (OpenCV, see `docs/lab/INPUT-DEBUGGING.md`); prove a click landed with `fb-react.py` |
...
| `/data/assets-staging` is root-owned inside CT950 and a DIFFERENT mount on labhost; `mkdir` fails, and a `cd` that fails in a `( cd $d && … )` subshell hashes whatever the cwd is (the a1000 wave wrote the shared clone's `MANIFEST.sha256` twice) | two mounts, one name; unguarded `cd` in a loop | stage media under `/data/vms/sandbox/<id>/media/` (both sides see it); `cd "$d" \|\| exit 1` in every subshell |
| Files a rig writes on labhost are unreadable from CT950 | `labrun` runs as root with umask 077 | `umask 022` is the first line of every script `labrun` runs |
| A host-native MAME station is BLACK forever in one mode and fine in another, on every graphics option and with the card removed | the fleet's MAME pin has a regression the machine's boot ROM trips (domainos: 0.289 spins in the DN3500's KEYBOARD self-test polling the SIO; Service mode is fine because it skips the self-tests) | sample the CPU's PC from a Lua autoboot BEFORE touching a patch file — it names the loop in one run, where hunk-by-hunk bisecting did not. Then pin the STATION, not the fleet: `NATIVE_MAME_TAG`/`NATIVE_MAME_BASE`/`NATIVE_BASE_PATCHES` in `native.d/<id>.sh` plus `-<tag>` patch variants |
| A bare MAME rig sits at 3% CPU with the ctlsock never acking, and looks like a dead daemon | a device with no good ROM dump raises MAME's startup warning panel, which PAUSES the machine until a keypress a kiosk never sends. A `-str N` gate run skips the panel, so the build gate and a race harness both "pass" | `skip_warnings 1` in the rig's `ui.ini` AND the skip-warnings patch (the ini alone is not honoured); suspect it whenever a rig is black but alive |
| Shifted characters arrive one key late (`Apollo DN3500` types as `APollo dN#500`) | the Shift field and the character field are paced independently by the ctlsock module, and Shift usually sits on a different port | raise `SH_KEY_MIN_HOLD_MS`/`SH_KEY_MIN_GAP_MS` until a line types byte-perfect twice (domainos: 80 fails, 180 works), then set `demoProgram.perCharMs` at or above the validator's drain rate (HOLD+GAP). Not the same knob as `MAME_CTL_KEY_EXCL`, which only applies to CPU-scanned matrices |
| A `new --like` station mangles punctuation nothing else mangles | the scaffold deep-copies the sibling's `keyboard.charMap` verbatim and nobody re-derives it (domainos shipped samcoupe's) | derive it from the station's OWN generated keymap — `mame-keymap.py`'s field names carry each key's legend (`8 (` means shift-8 is `(`), so the map is a one-command check, not a guess |
| A checkpoint `LOADST` returns OK and the emulated clock really rewinds, but the screen keeps the previous visitor's pixels | the driver registers no save state of its own, so video memory is not in the state file; a repaint-on-demand desktop never redraws it | `grep -c 'save_item\|save_pointer' src/mame/<sys>/*.cpp` BEFORE designing the reset — it is 0 for the whole Apollo driver. The fix is a per-driver save-state patch; do not infer save support from a driver's flags |
| A pointer's counts are applied (`STAT` shows them) and nothing on screen moves | the guest never sampled the port at all — apollo_kbd reads its mouse only outside `KBD_MODE_0_COMPATIBILITY`, which Domain/OS never leaves | attach `gdb -p <pid>` to the RUNNING binary with a `dprintf` call-count probe on the suspect setter (these binaries keep their debug info, so this costs no rebuild) and settle "is this code even reached" before tuning pacing, gain or re-latching |
| A frame loop written as `bash -c "… \$(printf …)"` writes ONE file named `f$(printf …).png` | shell quoting inside the one-liner | put the loop in a script file (`frames.sh`), never in a quoted `-c` string |
| `stations-registry.py new --like <x11 station>` refused (`missing qemu-streamhost.sh`), then left `SH_X11_CMD_FILE`, the x11 `--udp`, and `operator.labctl.udp_port` at the sibling's values | the scaffold assumed a QEMU sibling | fixed 2026-09-09 (a1000 wave): it takes the launcher named by `runtime.x11.launcher` and rewrites all three — check them on any entry scaffolded before |
| `smoke-rig.sh` refuses an x11/shm rig (no QMP socket) and its stream.env drops `SH_X11TEST_*` / points at the sibling's display | it assumes QEMU | bind a placeholder unix socket at `<rig>/qmp.sock` (python), then patch `stream.env` (display or `SH_CAPTURE=shm`+`SH_SHM_PATH`, input backend, keymap, FIFO) and re-run `<rig>/run-daemon.sh` |
| `smoke-rig.sh --down` leaves the smoke EMULATOR and its Xvfb running; a station that reuses that display then fails to start 3× at landing | `--down` stops the daemon only | kill the smoke emulator and Xvfb by pidfile BEFORE `station-land` when the station reuses the display |
| vitest red at landing: `tileWiring … has a keyboard profile` / `OS_FAMILY is missing tile` | the keyboard family map is `spa/src/ui/keyboard/keyboardProfiles.ts` (OS_FAMILY), not under `spa/src/data/` | add `<id>: '<family>'` next to the sibling's row in the ledger commit |
| A validate error shipped in a commit; the landing found it 10 minutes later | `validate 2>&1 \| tail -1 && …` masks the exit code | run every gate to a file and test `$?`; never pipe a gate |
| Second wave in one session: `duplicate bindingOrder=91` (and signal/manifest/golden/bringUp) at landing | two `new --like` scaffolds from one base pick the same next-free orders | before the landing merge, reassign every order to max+1 over all rows |
| After the union merge of the scene tables the OTHER wave's row lost its closing `keyboard/mouse` + `},` and `spa-scene-rows` said "no row for <id>" | the append-only union seam | grep the row below your seam; run `vitest src/scene` before committing the merge |
