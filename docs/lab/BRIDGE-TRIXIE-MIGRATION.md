...
| **2** | `bbcmicro` ✅, `armeval` ✅, `zx81` ✅, `dragon32` ✅, `oricatmos` ✅, `mpf2` ↩︎, `kc854` ↩︎ | The MAME-in-chroot tiles. **Five landed 2026-08-10** and the chroot swap was indeed mechanical; ccache carried 96-100% of every build after the first. `mpf2` and `kc854` rolled back on a second-cold-boot failure that is theirs, not the suite's. The bookworm chroot's biggest consumer is mostly retired, but it cannot be deleted until those two follow. |
