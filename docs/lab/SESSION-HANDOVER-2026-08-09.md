...
**55 production tiles**, all accepting stream tickets. Added this session:
...
All were told: research only, no tiles, teardown is part of done, and I write the
...
**Nothing needed building.** Previous r1847 — the revision the tile already
...
- **no m68k toolchain exists in the golden** — no `cc`, no `/usr/include` — which
...
- **Pointer metadata on every tile.** `stream.pointer.{present,absolute,method}`
...
- **Grid badge** for graphical-but-relative-pointer tiles (`--warn`, beside
...
- **`plus4`** registry pacing corrected to the 80/80 the tile actually runs.
...
2. **Consolidate the MAME builds.** Six of seven binaries pin the same
   ...
   **The operator's caveat is correct and stricter than they put it:** the golden
   ...
   is not merely stale, it can fault. Every affected tile needs a re-bake. For
   the six retro tiles that is `--force` on builders that already bake and
...
- **Push as soon as work is ready; deploy visual changes.** Standing operator
  instruction.
- **Deploying a tile needs THREE runtime documents**, not two: `tiles.json`,
  `gallery-manifest.json` **and** `golden-manifest.json`. The third is the
  allow-list for `POST /restore/<id>`; missing it leaves a healthy tile with a
...
- **One worktree per parallel agent; no file-ownership rules.** Operator's call:
  ...
  the same object.
- **Bake the state the machine itself chose**, and put affordances in the SPA
  around it — the `plus4` lesson, still the house style.
- **"No golden" read off a tile whose builder is still running is a timestamp,
