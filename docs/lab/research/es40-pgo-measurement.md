...
- Arms **interleaved** (ctrl, pgo, …) to cancel drift from the live tile's
  shared-box load.
...
- **Gain is marginal (~1–2%)** on a tile that already meets its performance
...
- **Packaging cost is real and ongoing.** PGO turns one `make` into a
  ...
  needs a golden and a headless workload runner, and the profile goes
...
from this measurement — run in the es40 `src/` dir with the box lib tree on
`-L`/rpath:
...
# from a golden copy, wait for the 1280x1024 desktop, then SIGTERM (graceful
