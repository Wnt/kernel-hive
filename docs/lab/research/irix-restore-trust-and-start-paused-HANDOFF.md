...
2. **Phase 2 — true start-paused.** `systemctl start streamhost@irix` should
   end with the guest AT the golden state, SIGSTOPped, ~0 CPU, waking on the
...
- **w2kalpha idle auto-pause ON** — es40 fork `fc82f05` `host_freeze_reanchor`
  absorbs host-side freezes (zero cc billed; interval timer, TOY/RTC, ACPI PM
  timer re-anchored). Proven by 6-min and 25-min SIGSTOP soaks with apps open.
  Repo `8ba52b6`; `docs/guests/w2kalpha.md` §idle-pause.
- **Fleet instant-ready** (`cbff4cb`, `d64456e`): 55 QEMU loadvm tiles launch
  `-loadvm golden -S` → restored-but-frozen; first session's unconditional
  `cont` (idle.rs) wakes them. Census: 53 frozen, only amiga/daybreak/
...
(issue #44, 2026-08-04)" — the audit, the patch, the bake/restore flow, the
...
1. **Soak the restore path adversarially** on a namespaced clone (or the
   production tile stopped — operator prefers the least-work rig; clones only
...
**Fork/build/bake mechanics (the rebuild-orphans-states trap):**
...
- Fork `github.com/Wnt/mame` branch `irix`; patch stack under
  ...
  stack); builders `scripts/build-guests/emulators/build-mame-irix.sh` (box)
  / `build-mame-macos.sh`. Production binary
  `/data/vms/streamhost/assets/irix/mame/sgi`.
- `provenance-golden.md5` binds the golden state to the exact binary md5 —
  **ANY rebuild orphans every state** (registration signature) and the
  launcher then cold-boots loudly. After a MAME fix: rebuild → REBAKE
  (`scripts/build-guests/irix/irix-savestate/bake-golden.sh`, tile stopped;
...
- Measurement rules: `docs/lab/MEASUREMENT-METHODOLOGY.md`; bench numbers are
  epoch-bound; the tile is core-pinned (`IRIX_CPUS`); `-frameskip 6` is
...
- Launch restores (`IRIX_STATE=golden`) and RUNS; `SH_IDLE_PAUSE_WARMUP_SECS=780`
  holds the first freeze because **livewatch's pointer probe is the only
  ...
  it cannot probe a frozen guest. Freeze-at-start today ⇒ budget silently
  expires ⇒ every launch becomes a running 390 s cold boot.
- Watchdog interplay is already solved: `mame_stopped()` — both watchdogs
  stand down while SIGSTOPped; labctl thaws before driving; the daemon
  reconciler re-freezes within grace+heal.
...
- Launcher: restore → SIGSTOP immediately (do NOT increment `.state-tries` at
  launch — a frozen guest exposes nothing, so there is nothing to vet yet).
...
- Mind the timing conflict: the idle pauser re-freezes 60 s after the visitor
  ...
  clear) or give livewatch one deferred-freeze window after first wake.
  If phase 1 ends with "restore trustworthy", this whole knot shrinks: keep
  the budget as a formality, freeze at launch, warmup → 0.
...
- Daemon side needs NOTHING: `session_started` already conts unconditionally;
  a launcher-frozen guest converges the same way the QEMU `-S` tiles do
...
(`labctl reset irix` = service restart) still lands frozen-at-golden;
...
- Deploy: mirrored files go through `scripts/dev/box-sync-push.sh` (the
  ...
  pushing main.
- A parallel agent was active on the box today (soltest rigs `debridge-7f3a`,
