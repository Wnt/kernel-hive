...
Written 2026-08-11. The w2kalpha tile (Windows 2000 RC2 for Alpha, es40) is a
non-QEMU streamhost tile modelled on IRIX: es40 runs headless (no X server, no
...
- **Clean 1280x1024 golden** at `/data/vms/streamhost/assets/w2kalpha/nt.img`
...
- **Production assets staged** `/data/vms/streamhost/assets/w2kalpha/`:
  ...
  tree, so the tile does NOT depend on the retired soltest scratch area).
- **Tile runtime** `/data/vms/streamhost/tiles/w2kalpha/`: `w2kalpha-runtime.sh`
  (reflink-copies the golden per launch → cold-boots es40 headless + serial
  ...
  staged golden to a 1280x1024 desktop published on shm, and a keyboard verb
...
input testing. The golden (m5-1280 / the staged assets) is a separate clean
snapshot, unaffected.
...
1. **Golden guest polish, then re-capture.** On a cold-booted golden, over the
   ...
   - Confirm autologon-to-desktop still holds, then **clean OS shutdown** and
     re-capture `nt.img` → restage as the golden. A cleanly-shutdown disk also
...
2. **Registry entry + streamhost/SPA integration.** `scripts/tiles-registry.py
   ...
   shm/mamesock tile): id/tileDir=w2kalpha, lifecycle candidate→production,
