...
- **Image**: the official **`haiku-r1beta5-x86_64-anyboot.iso`** (1.41 GB;
  ...
  *(Superseded: the live station now boots a persistent installed disk,
  `tiles/haiku/haiku-persist.qcow2` with an internal `golden` snapshot —
  see the station launcher / `scripts/serve/golden-manifest.json`.)*
...
- License: **free/open (MIT)** — the preferred faithful path for a BeOS station; no
...
## How the live station was wired in the neko era (concurrency-safe, isolated project)

To avoid clobbering the sibling-edited `docker-compose.gallery-guests.yml`, the neko-era
:8107 station ran as its **own** compose project (same pattern as Sailfish/TempleOS):
...
- File in CT 110: `/opt/osgallery/docker-compose.haiku.yml` (service `haiku`,
  ...
  `/dev/kvm`, `NEKO_SESSION_IMPLICIT_HOSTING=true` so opening the station grants
...
superseded by the streamhost station before it (see the banner at the top).
...
- **Landing screen**: the station lands on the "Welcome to Haiku!" first-boot dialog
...
- **`--arch x86_gcc2h`** builds the 32-bit gcc2-hybrid ISO instead (the "classic"
  BeOS build) if the UI wants the most period-accurate variant; boot path identical.
