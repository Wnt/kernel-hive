# TinyCore gallery station — scene checkpoint + ssh exec channel

**Status: LIVE.** Station `tinycore` (VMID 82, udp/54082) — Tiny Core Linux x86
GUI LiveCD (FLWM + wbar desktop), scene checkpoint with an open aterm
...
- **Model**: the LiveCD runs entirely in guest RAM — **no installed disk, no
  ...
  `savevm golden` checkpoint; the guest never boots or uses it. The
  whole scene — loaded `openssh.tcz`, sshd, DHCP lease, open terminal, xset
  tweaks — lives in the checkpoint's RAM/device state.
...
- Display/audio *backends* differ freely between capture (`-vnc`/`none`) and
  production (dbus) — backends are not part of vmstate (verified for this
  station 2026-07-06 and again by the builder's own round-trip).
...
- **The streamhost transport runs `SH_POINTER=abs`**, but because Xvesa reads a
  ...
  **relative tracking with edge re-homing**: the cursor clamps at the screen
  ...
  LIVE station via a registered browser client — no QMP/HMP offline injection
  moves the cursor under the production dbus launcher. Derivation: drive the
  client across a mid-screen grid with `SH_DEBUG_INPUT=1`, read `recv`/`inject`
  from the daemon log, locate the guest cursor via `labctl shot`, fit
  `landing = A·inject + B`, set `scale = 1/A`. **Re-verify after any TinyCore
  major bump or resolution change.**
...
- `openssh.tcz` (+deps) tce-loaded from the version-matched upstream repo,
  ...
  gallery pubkey (`/root/.ssh/gallery_guest_key.pub` on the labhost) as
...
6. PROVES: idle determinism (two shots 5 s apart byte-identical) →
   ...
   still answers.
7. Artifacts: `state.qcow2` (checkpoint inside), `fixture-golden.png`,
   `BUILD-INFO.txt`. Idempotent: existing checkpoint → no-op (`REBAKE=1`
   recaptures); ISO re-download skipped on md5 match.
...
- The live launcher `tiles/tinycore/qemu-streamhost.sh` is the hand-written
  checkpoint launcher: create-if-missing `state.qcow2`, conditional
...
- **Do not use resetMode=restart**: a reboot re-runs the LiveCD boot and lands
  on a bare desktop with no terminal, no ssh and no tweaks — the scene only
  exists inside the checkpoint.
- The capture **must** go through text mode: the GUI has no terminal open and no
...
- `tce-load -wi openssh` needs internet at capture time (the tcz repo is not on
