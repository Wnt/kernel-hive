# Commodore Amiga 500 + Workbench 1.3 — gallery tile notes (:8118)
...
auto-boots **Workbench 1.3** off a real **Kickstart 1.3** ROM. This is an
**"emulator bridge"** tile — streamhost captures the Linux framebuffer + AC97 audio
...
the same desktop), `streamhost-live.png` (the live tile under streamhost@amiga).
...
Win9x, NeXTSTEP tiles). The only rule: don't re-distribute those copyrighted binary
...
## LIVE TILE STATUS (2026-07-08) — LIVE at udp/54118, Workbench 1.3 desktop CONFIRMED, Paula non-silent
...
- **Golden fixture:** boots straight to the Workbench desktop via `-loadvm golden`
...
1. **FS-UAE is NOT in the frozen bridge base** (this tile postdates the base freeze).
...
- **`--fullscreen=0` + fixed `window_width/height`** — real fullscreen renders **BLACK**
  ...
  tile). A **WINDOW** (720x568) on the bare-X root captures correctly (centred, black
  border — cosmetic).
...
- **`ALSOFT_DRIVERS=alsa`** — force OpenAL to the ALSA backend so Paula deterministically
  reaches `default` -> `hw:0,0` -> AC97 (no PulseAudio on the box).
...
Restore the live `asound.conf` (`type plug; slave.pcm "hw:0,0"`) before the golden bake.
...
- SPA web port (reserved for integration): **8118**. VMID **218**.
...
```
Golden bake/verify (HMP over QMP): `savevm golden` / `loadvm golden` / `info snapshots`.
...
- **Pointer:** `SH_POINTER=abs` via usb-tablet. Workbench is mouse-driven; the abs tablet
  ...
  streamhost cursor drives it directly; end-user click calibration through the SPA is a
  follow-up (not blocking; not yet exercised), same status as the C64 tile.
...
- **Fallback not needed:** the task allowed a KS3.1 + WB3.1 "prettier" fallback if 1.3
  was troublesome — it was NOT; KS1.3 + WB1.3 built and verified cleanly, so the tile
...
A short-lived "cold-boot-on-visit" pilot (box `amiga-coldboot-watch.service`, a guest
`amiga-emu` supervisor, a modified `/etc/bridge/launch.sh`, and a coldboot-aware tile
launcher) broke the tile: it left the golden RAM snapshot on a **bare-X** moment (fs-uae
...
- Box watcher service removed; guest `/etc/bridge/launch.sh` restored from
...
- Tile launcher restored from `qemu-streamhost.sh.pre-coldboot.bak` (byte-identical to the
  canonical device set below); `tile.env` restored from `tile.env.pre-coldboot.bak`
  (dropped the coldboot `SH_IDLE_PAUSE_SECS=0` line — bridge tiles c64/atarist don't set it).
- **Golden re-baked** on a clean Workbench 1.3 desktop: started fs-uae in the live kiosk X
  session (as user `bridge`, `DISPLAY=:0`), let it auto-boot Workbench, then HMP
  `delvm golden` + `savevm golden` over the tile qmp.sock (new golden 814 MiB, device set
