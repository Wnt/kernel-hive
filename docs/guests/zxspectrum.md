...
**Status: LIVE production streamhost station** (slot 127, UDP 54127, VMID 230,
kiosk SSH 5830). Emulator-in-captured-Linux **kiosk** in the family of
...
framebuffer and the AC97 audio, exactly like every other kiosk. The station
...
`-verifyroms` demands every one. The station pins exactly one (`-bios en`), so the
...
### 4. A kiosk MAME can be born DEAF, and a checkpoint can capture it

The first checkpoint of this station was pixel-perfect and completely unusable: after
...
builder therefore gates the capture on a **behavioural** predicate, not a pixel
...
After the capture the builder types into the **restored** scene as well, because
"restores a picture" and "restores a machine you can type on" are different
claims and this station has already failed the second one once.
...
- **MAME does not reach real time in this guest.** `-video none -sound none
  ...
  frames than the arithmetic assumes, and the shortfall tracks host load (labhost
  was at load 77 on 16 cores during these runs).
...
twenty-three distinct letters, three trials per pacing, labhost at load ~40–55:
...
hand. To rebuild from the frozen seed:
...
the checkpoint and requires a recapture.
