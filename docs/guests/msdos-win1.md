# msdos-win1 tile — MS-DOS 6.22 + Windows 1.01 (:8113)

**One image, TWO SPA exhibits.** This single `msdos-win1.qcow2` boots to the
...
> tile **`msdoswin1`** — see its stanza in `streamhost/tiles-manifest.sh`
...
screendumps, then promoted into the live golden on 2026-07-14 (see "Option B —
...
### 1. Mouse doesn't work — FIXED on clone (needs golden rebake; NO device-set change)
...
- **Option A — minimal (issue 2 only, lowest risk).** Bake **SETVER** into the live
  golden (add `SETVER.EXE`, `DEVICE=` line, the two `SETVER` entries) and re-save
...
- **Option B — complete (issues 1+2, recommended).** Replace the prebuilt `C:\WIN10`
  ...
  post-bake framebuffer gate. The DOS side (gallery menu, KEEN/COSMO/DOOM games) is
  independent and unaffected.

### Option B — PROMOTED LIVE 2026-07-14 (user-approved; guest golden done + framebuffer-gated)

The complete fix (issues 1+2) is now **baked into the live `msdoswin1` golden**.
Built on a namespaced clone (`/data/vms/soltest/win101-integ`, since removed) from
`disk_install2.qcow2`, framebuffer-gated on the clone AND on the live tile, then
swapped in with a timestamped backup.

- **What changed in the golden** (`/data/gallery-guests/MSDOSWin1/msdos-win1.qcow2`):
  ...
  - SETVER already baked: `C:\DOS\SETVER.EXE`, `DEVICE=C:\DOS\SETVER.EXE` in
...
- **Framebuffer proof (LIVE tile):** `labctl shot` → genuine Win 1.01 Executive;
  ...
  Executive.
- **Backup / rollback.** Pre-swap live golden saved to
...
- **GOTCHA — "input dead" is idle auto-pause, not a guest bug.** The shared
  ...
  keyboard regression.
- **Still host-side (not the guest golden):** issue #4 (splash mid-blit streaking) is
  ...
  path + the SPA `pointerRel` flag (main chat) — both since shipped
  (commit `25e27b3`) and browser-verified 1:1 live 2026-07-14.

## Streamhost fixes 2026-07-13 (keyboard / mouse / sound)

Three user-reported issues on the live `msdoswin1` streamhost tile were
...
1. **Keyboard "stops working" in Windows 1.01 — FIXED (live).** Windows 1.01
   ...
   always worked. Fix is host-side, **no golden rebake**: a per-tile
   ...
   NumLock is OFF in the golden, so the keypad codes act as cursor keys. Gated by
...
2. **No mouse cursor in Windows 1.01 — NOT shipped (blocked; needs golden rebake +
   ...
   re-SETUP selecting *Microsoft Mouse → Com1* to rebuild `WIN.COM`, then a golden
   re-bake with the new device set. Deferred for green-light — the serial device is
...
3. **No sound in Commander Keen (MS-DOS) — FIXED (live).** Keen 1 *Marooned on
   ...
   `-audiodev`). Fix mirrors the freedos tile but PC-speaker-only: launcher now has
   ...
   `tile.env` sets `SH_AUDIO=on`. **No golden rebake** — `pcspk-audiodev` is a
...
tile and isn't MS-DOS; Win 1.01 needs real MS-DOS-family binaries beneath it).
...
tiles; the only rule is not re-distributing the copyrighted binary media via the
GitHub repo.
...
## Canonical compose service (live on the host)
File on host inside CT 110: `/opt/osgallery/docker-compose.msdoswin1.yml`
...
UDP EPR range `53360-53379` was chosen clear of all existing tiles (max in use
...
- **curatedMetadata:**
  - MS-DOS 6.22 — Microsoft, 1994. Last standalone retail MS-DOS. 16-bit real-mode
    ...
    Iconic era software: EDIT, QBASIC, DOS games (Doom/Duke via the FreeDOS tile).
