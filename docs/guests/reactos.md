# ReactOS gallery tile — merge notes (:8106)

> **Historical (neko-era) wiring below.** ReactOS runs today as the streamhost tile
...
```
# LIVE tile profile (KVM, 2026-07-04 perf flip):
...
- **PS/2 kbd+mouse only** *(neko-era guidance — superseded)* — a boot-time `-device usb-tablet` could stall ReactOS 0.4.x USB enumeration on that setup; the live **streamhost** tile runs `usb-tablet` (absolute pointer) without issue — see `streamhost/tiles-manifest.sh`.
...
## neko-qemu tile env (neko-era) — was deployed as isolated compose project `osgallery-reactos`

File on host was `/opt/osgallery/docker-compose.reactos.yml` (mirrored the TempleOS/SailfishOS isolation pattern — never touched the concurrently-edited `docker-compose.gallery-guests.yml`). Was brought up with:
...
**Port block:** host `8106`, EPR `53340-53359` (53320-53339 was taken by a sibling agent — picked the next free block).
