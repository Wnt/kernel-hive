# SGI Indy, MIPS R4400 — IRIX 6.5 under Iris (`indyr4400` tile)

The gallery's **second** SGI Indy, and the first tile to run
[Iris](https://github.com/techomancer/iris) (BSD-3), a userspace Rust emulator
of the Indy. It is a deliberate pair with the [`irix`](irix.md) tile rather
...
| Runtime | **x11 runtime, BARE METAL** — MAME's Indy emulation kernel-panics under a KVM vCPU | ordinary **KVM bridge tile** — Iris is pure userspace and has no such constraint |
...
| Reset | `relaunch` (+ baked MAME savestate) | `loadvm golden` |

Both run the same IRIX 6.5.22 install — this tile's disk is literally derived
from the `irix` tile's golden CHD (see *Media* below).

## Status

**Live production tile** (added 2026-08-10). Slot 136, UDP 54136, ssh 5839,
...
- Login: `demos`, no password (see also `root`, empty password, from the
  `irix` tile — the same install).

## Media and provenance

The IRIX disk is **not** a new download. It is this lab's own
`irix65-apps.chd` (the `irix` tile's golden) extracted with
...
The tile attaches the asset as a second, read-only virtio drive so the 6.3 GB
never enters the tile's qcow2 overlay. Iris sizes a SCSI disk with
...
own writable disk — inside the tile overlay, and therefore inside the `golden`
snapshot. Read-only drives are invisible to `savevm`, so the golden stays
...
installed in the tile**, and `CARGO_TARGET_DIR` is unset inside the chroot so
the build does not touch the box's shared cargo target tree. Measured build:
...
The bridge kiosk runs a bare X root with no WM, so nothing ever calls
...
focused. Every other bridge tile is SDL and does not care.
...
devices. This tile has no absolute host device (no usb-tablet — Iris wants
...
SPA drives it through Pointer Lock (the derived `relativePointerOnly` grid
...
`resetMode: loadvm`, snapshot `golden`, baked at the **Indigo Magic Desktop**
...
Cold boot (no golden) takes **~7 minutes**: PROM, IRIX autoconfig — IRIX
...
is why the tile ships `-loadvm golden`.
...
| tile QEMU (host view) | ~150 % CPU, RSS ~1.15 GB |
| overlay.qcow2 | 702 MB (1.1 GiB vmstate inside it) |
| asset | 6.4 GB apparent / ~500 MB allocated (sparse) |

Iris is the most expensive emulator in the lineup, so this tile deliberately
...
- **Networking is parked.** IRIX reports `ec0: machine has bad ethernet
  ...
  from Iris's monitor console on `127.0.0.1:8888`) baked into `nvram.bin`. It
  ...
  login, so it is not in the golden.
...
- `reset.mouse` / `reset.keyboard` are `UNVERIFIED` in the registry: both are
  proven by framebuffer here, but not yet through the real SPA in a browser.

## Rollback

The tile is self-contained. `systemctl stop streamhost@indyr4400`, kill the
QEMU by `/data/vms/streamhost/tiles/indyr4400/qemu.pid`. Nothing it touches is
shared except the frozen bridge base (read-only backing) and the `irix` tile's
