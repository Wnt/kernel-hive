# Commodore VIC-20 (PAL) — gallery station notes (udp/54085)
...
`READY.` prompt. An **"emulator bridge"** kiosk — streamhost captures the Linux
framebuffer + AC97 audio (the VIC-I sound routed through ALSA) exactly like
every other station. See **`streamhost/docs/BRIDGE.md`**.

**Shared seed:** `/data/vms/bridge/bridge-base-trixie.qcow2` (read-only backing; built
by `scripts/build-guests/lib/bridge-base.sh --suite trixie`). It already contains the whole VICE
family, `xvic` included — the seed builds VICE from source for the `c64` station
...
This is the cheapest station in the lineup to reproduce: **no ISO, no cartridge, no
disk image, no licensed ROM to fetch.** VICE bundles the Commodore ROMs (which
is exactly why Debian cannot ship VICE and why the bridge seed builds it from
...
builder's only external input is the frozen bridge seed.
...
- **Reset mode:** `loadvm`, internal checkpoint `golden`. No post-restore keys.
...
Identical in shape to its kiosk siblings (`c64`, `apple2`, `atarist`, `amiga`,
`mpf2`), which is what makes the checkpoint portable and the launcher parity claim
honest:
...
- **800×600, not the seed's stock 1024×768.** VICE's SDL window is a fixed size
  ...
  exactly as the `c64` station does. The captured frame is then mostly picture,
...
correct and harmless for MAME. Copy that line for a VICE station and the emulator
...
single byte of output. Confirmed by gdb backtrace on the station, 2026-08-08:
...
seed profile and the `c64` station do the same, which is precisely why they work.
...
`c64` station hit it on the C64 BASIC ROM. The frozen seed's
`/usr/local/share/vice/VIC20` is missing `basic-901486-01.bin` the same way, so
`vic20.sh` repairs the set from the source tree the seed retains
...
why 40/40 works most of the time. This labhost runs 30+ emulators; when VICE's
...
at any station that declares hold == gap (vic20, amstradcpc, mpf2). That is a
...
| Artifact | Shows |
|---|---|
| `ready-before-golden.png` | the untouched cold-boot power-on screen that was captured |
| `keyboard-print3.png` | `PRINT 3` typed through QMP at the station's production pacing, with `3` printed back by BASIC |
| `golden-restored-after-keyboard.png` | `loadvm golden` returning to the exact captured scene after the keyboard test dirtied it |

The checkpoint is captured from an **untouched** cold boot and the keyboard proof runs
**after** the capture, against the restored scene, so nothing the proof types can
ever reach the checkpoint — the mpf2 add shipped a checkpoint carrying its own
verification output and had to be recaptured.
