...
## What the machine is — and why it is not the tile next door
...
banner, and so is the `cbm8032` tile's. At a glance they are close. What makes
...
the 8032 has the stronger claim to the shelf on merit, and this tile's case
...
the C64 tile and the VIC-20 tile).
...
at the prompt through the SPA's on-screen keyboard; `PRINT`, `BANK`, `LIST` and
the built-in machine-language monitor all work from cold.

This follows the lesson the `plus4` tile paid for: **bake the state the machine
itself chose.** An earlier plus4 golden rested inside an application and dropped
...
area of every other bridge tile (`c64`/`vic20`/`plus4` all run 800×600), to
...
running tile's QEMU is **≈660 MB RSS** (the daemon's rss-guard arms at
anon 596 MB + 2048 MB). If a future change pushes MemAvailable under the floor
the build fails and says to raise `MEM` to 1024 and re-bake.
...
none in 22). **Two frames is a floor, not an answer.** Re-bisect if this tile
...
| `keyboard-print3.png` | `PRINT 3` typed at 80/80 **after** the bake: BASIC echoes the line, prints ` 3`, paints a second `ready.` |
| `golden-restored-after-keyboard.png` | `loadvm golden` returning to the exact baked screen, the typing gone |
