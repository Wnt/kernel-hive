# AmigaOS (AROS) gallery tile — notes for the orchestrator
...
- **Alternative (NOT used):** classic 68k AmigaOS 3.x via FS-UAE / vAmiga needs a
  ...
  is faithful to the Workbench look. If a future "real" AmigaOS tile is wanted,
...
## Live tile (own compose project, isolated like sailfish/templeos)
...
## Port / EPR allocation
- HTTP tile: **8110**  (existing tiles occupy 8081-8107; 8110 was free)
...
- **KVM ADOPTED (2026-07-04 perf rollout, test-then-adopt PASSED).** Flipped
  ...
  the LIVE tile: Wanderer/Workbench desktop renders on a fresh boot; **input reaches
...
- **LIVE PROMOTION (2026-07-15/16 UTC).** Backed up the PS/2 golden, launcher,
  and env to `/data/vms/streamhost/backups/amigaos-pre-abs-20260716T0237Z`, then
  re-baked and emitted the registry-backed candidate. The live process has
...
- **Audio buffer knob is already gallery-wide** (baked into launch-qemu.sh:
  `-audiodev pa,id=snd,out.buffer-length=100000,out.latency=50000`); no per-tile
...
- **Framing / RESOLUTION (RESOLVED 2026-07-06):** AROS's GRUB `boot/grub/grub.cfg`
  ...
  desktop. The tile previously captured at the gallery-default `640x480`, which — with
...
- Archetype for the SPA: an **Amiga wedge** (e.g. Amiga 500/1200 wedge-with-CRT).
  No existing SPA archetype fits perfectly; the beige-tower-crt is the closest
