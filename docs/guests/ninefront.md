# ninefront — 9front (Plan 9 fork), tile notes

- Tile: `ninefront` (VMID 96), q35/`-cpu host`, 1920x1080 `-vga std`, intel-hda,
  disk `/data/gallery-guests/9front/9front-11554.amd64.qcow2` (internal `golden`
  snapshot; launcher boots `-loadvm golden`, savevm persists — no `-snapshot`).
...
position. Framebuffer-verified on a soltest clone and the live golden: a 18 s
...
- Baked into BOTH the golden disk and the golden RAM snapshot. The settled
...
- Network: SLIRP via virtio-net; in-guest `ip/ipconfig` (DHCP 10.0.2.15) +
  `ndb/cs`; fetch from the box with `hget http://10.0.2.2:<port>/...`.
  The baked termrc now brings the net up at every cold boot.
...
- Clean shutdown for re-bakes: `fshalt` (syncs cwfs and exits QEMU).
...
re-res is a **cold-boot re-bake** (not a `loadvm`): patch `plan9.ini`, cold boot,
re-lay-out the fixture at the new geometry, `delvm`/`savevm golden`. Built ON TOP
of the current coalescing golden disk (the `/amd64/bin/warpd` binary is untouched
...
collection. The golden keeps the uncluttered four-window composition above; the
...
rio from framebuffer truth, recreates the same four-window fixture, and then lets
the normal recorder freeze/save/verify seam invariant run. Publish with
`postprocess-boot.sh`, `trim-boot.sh`, and `gen-boot-manifest.sh` as documented in
`scripts/coldboot/README.md`.

Re-record on the box (the `bootrec-tap` companion binary must be built first — it
...
Re-recorded at **1920×1080** on 2026-07-27 (seams with the 1920×1080 golden):

- record clone: cold boot → four-window rio fixture laid out → warpd pointer-park
  at `1580,916`; freeze/`savevm golden`/verify `SSIM(golden-first-frame,poster)=
...
- live seam (boot.mp4 last frame vs `labctl reset ninefront` → `loadvm golden`
  ...
  match between two independent bake instants: `games/catclock` `0.9916` and the
...
- **pre-1080 golden (the exact 1024×768 coalescing golden, before the 2026-07-27
...
- pre-coalescing golden (before the 2026-07-26 latest-wins agent swap; the
