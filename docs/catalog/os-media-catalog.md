...
| BeOS R5 Professional 5.0.3 | https://archive.org/details/beos-5.0.3-professional-gobe (✓; the `beos-professional-edition-5.0.3` item this row used to point to no longer exists) | preservation (Pro edition; PE was freeware) | bin/cue, three MODE1/2352 tracks (track 1 bootable ISO 9660 `BeOS_Tools`, track 2 raw BFS `BeOS 5 Pro Edition` system volume, track 3 other/PPC) | none | ~772 MB | small | works-known | 5 |
| Rhapsody 5.1 DR2 (Intel) | https://archive.org/details/rhapsody5.1 (✓, `Rhapsody_5.1/`: boot + drivers floppies, raw CD, all .gz; md5s published) | preservation (commercial Apple, no freeware re-release — same posture as NeXTSTEP 3.3) | 2× 1.44 MB floppy + 630 MB raw CD (`RhapsodyDR2`) | none | ~250 MB (gz) | small | works-known (station `rhapsody`, /opt/qemu-rhapsody) | 5 |
...
- **BeOS R5**: split the bin/cue into per-track 2048-byte images (no `bchunk` on this box; a small Python script does it) — QEMU cannot present a multi-track disc, so install is done by copying the track-2 BFS system volume's files (with attributes) onto a fresh MBR-partitioned disk via a Haiku helper VM, not by running the disc's own Installer. The real blockers are NOT the boot menu's "disable BIOS calls" toggle, which covers neither of them: (1) R5's ISA `config_manager/isa` add-on calls the 16-bit PnP BIOS entry point, which SeaBIOS doesn't implement — page-faults the kernel inside `input_server`'s driver scan, so the desktop never gets a cursor or Tracker; remove the add-on (`config_manager_off/isa`). (2) R5's kernel GPFs in the idle thread under KVM with `-cpu pentium2/pentium3` (works fine under TCG `-cpu pentium3`) — ship on TCG only, like `os2warp`. Set VESA mode `1024 768 16` and serial-debug settings in `config/settings/kernel/drivers/`, but they are only honoured when the volume is booted directly (`-boot c`); booting the CD's loader with the partition selected ignores them. Cap **RAM ≤768 MB** (R5's kernel ceiling); NIC = `ne2k_pci`. The deliberate "original behind Haiku" pairing.
- **Rhapsody 5.1 DR2 (Intel)**: installer floppy → Device Drivers floppy → page 3 option 4 “Intel PIIX PCI EIDE/ATAPI (v5.01)” → Mach kernel from the CD; installs to one IDE disk. On stock QEMU ≥ ~1.0 sustained IDE I/O dies with `hc0: interrupt timeout … Resetting drives` (the community's “endless EIDE lockups”): a **guest** bug — Mach never EOIs the master 8259 on a spurious cascaded IRQ15, which QEMU's batched IRQ delivery provokes on the first timer/IDE coincidence. Ship on the station-specific `/opt/qemu-rhapsody` (kernel-hive fork + `streamhost/qemu-patches/0006-i8259-lenient-spurious-cascade.patch`, `KH_I8259_LENIENT_CASCADE=1`). TCG, `-cpu pentium2 -m 64` (kernel won't boot above 192 MB), serial Microsoft mouse only (`-chardev msmouse -serial chardev:`), `ne2k_pci`. Details: `docs/guests/rhapsody.md`.
...
- **atari800xl (host-native, 2026-09-08)**: an Atari 800XL (PAL) on MAME's `a800xlp` driver with a 1050 drive (`-sio a1050 -flop1 hive.atr`), no bridge — the successor to the "Atari 800/XL + Atari BASIC" row above, which assumed atari800-in-a-bridge and a bare BASIC prompt. The boot disk is a **MyDOS enhanced-density ATR whose boot sectors are MyPicoDos 4.06** (HiassofT), so the machine powers on into a file menu: Boulder Dash, Dropzone, River Raid, Star Raiders, The Last Word 3.2 (80-column word processor, freeware) and an `ATARIBAS.XEX` stub back to Atari BASIC. Sources are pinned by sha256 in `scripts/build-guests/tiles/atari800xl.sh`; the wave ledger is `docs/lab/ATARI800XL-WAVE.md`. **A double-density image will not boot** — an Atari 1050 is SD/ED only, so 130 KB is the hard ceiling for one drive, and titles distributed only as bootable ATRs (Yoomp!, Rescue on Fractalus!) cannot go on the menu at all.
