...
## Unix workstations — Solaris 10 (CDE) / Tru64 UNIX / BeOS R5 (retronet ICQ)

Narrower than the sections above: not a general software-stocking list for
these tiles, just the one retronet cross-reference. Solaris/Tru64 run
QEMU-TCG / es40-TCG with no GPU (same rendering-reality rule as everywhere
else in this doc), and CDE is already period-correct chrome — the terminal
client opens in a `dtterm` on the existing desktop, no new UI surface
needed. BeOS R5 is native Be-API, not X11/CDE.

| Title | Type | Source | Legal | Install | Why |
|---|---|---|---|---|---|
| climm (formerly micq/mICQ) 0.6.4 | util | [SourceForge `climm`](https://sourceforge.net/projects/climm/files/climm/climm-0.6.4/climm-0.6.4.tgz/download) | free (GPLv2 + BSD-ish pre-0.4.9 code, OpenSSL-linking exception) | build from source, `./configure --disable-ssl --disable-tcl --disable-otr --disable-peer2peer && make`; Solaris needs `PATH=/usr/sfw/bin:/usr/ccs/bin:$PATH` for its bundled gcc 3.4.3, Tru64 needs `CONFIG_SHELL=/bin/ksh` for its native Compaq C | the retronet ICQ fleet's Unix-side **terminal** OSCAR client (`docs/lab/RETRONET-BRIEF.md` §5, `docs/lab/retronet/ICQ-ONBOARDING-PLAN.md` Tier C) — console UI in a `dtterm`, same OSCAR/TCP-5190 protocol family as the Windows stations' ICQ 2000b. **Both Unix stations ran climm until 2026-08-22: `solaris` now runs Pidgin (row below) and `tru64` now runs Gaim 0.59.9 (row below that)**: the built binary, its SSI patch and `/.climm/` are deliberately left installed on the solaris golden as the documented rollback, so this row's build recipe stays live as the documented rollback for both, and for any other Unix station that wants a terminal client. Chosen over centericq/licq for buildability (no ncurses/Qt/OpenSSL required). Full sourcing detail, build strategy and the candidate comparison: `docs/lab/ASSETS-MANIFEST.md` §"climm (formerly micq)". |
| Pidgin (Gaim) 2.10.4 | util | already on the golden disk: `SUNWgnome-im-client` (Oracle JDS package, `/usr/bin/pidgin` + `/usr/lib/libpurple.so.0.10.4` + `/usr/lib/purple-2/liboscar.so`) — no download | free (GPLv2, Pidgin/GNOME project) | **nothing to build or install** — `pkgchk -l -p /usr/bin/pidgin` → `completely installed`; swap = author `/.purple/accounts.xml` (protocol **`prpl-icq`** — *not* `prpl-oscar*`; UIN `30000`, `server`/`port` settings `10.99.0.2`/`5190`, plaintext `<password>`, auto-login as **`<setting name='auto-login'>` inside `<settings ui='gtk-gaim'>`**, and **`authorization`=0** — "Require authorization" defaults ON and silently blinds the greeter to the station), set the Buddy List geometry and turn OFF auto-away in `/.purple/prefs.xml`, point `9999.golden-fixture` at `/usr/bin/pidgin` instead of `dtterm … climm`, recapture the golden | **LIVE on `solaris` since 2026-08-22, replacing climm** — a real GTK2 desktop app (buddy-list + chat windows, not a terminal), confirmed at the binary/source level: `liboscar.so` strings carry `feedbag` / `ssi: activating server-stored buddy list` (full SSI roster support, same non-negotiable the fleet's climm/ICQ-2001b clients meet) and libpurple's `autorecon` core plugin (always-compiled-in exponential-backoff reconnect). Recon + evidence: `docs/lab/ASSETS-MANIFEST.md` §"solaris GUI OSCAR client — Pidgin is ALREADY on the golden disk"; the as-built station is `docs/lab/retronet/ICQ-STATION-solaris.md`. |
| IM Kit (OSCAR/`ICQ.cpp`) — BeOS `beos` station, **NOT BUILT, patch plan scoped** | util | [github.com/HaikuArchives/IMKit](https://github.com/HaikuArchives/IMKit) source, HEAD `9c80ad1`, archived at sha256 `4eb6f38c3417dc6cb99610bd02fd86b32013f938b574d31462e1bb2221bd34e0`; rejected candidate ICBM/BeCQ (`icbm.8k.com`, R5 binary recovered via Wayback CDX) confirmed legacy pre-OSCAR ICQ v2-v5 UDP, no SSI — wrong protocol generation; secondary candidate [github.com/HaikuArchives/BeAIM](https://github.com/HaikuArchives/BeAIM) at sha256 `fb3f2a63dba29943207d124d44e27448814c1ea5abe2edbb876fc16fa6b0cc87` | free (IM Kit: BSD, "Copyright (c) 2004-2009 IM Kit Team"; BeAIM: freeware, author disclaimers only) | **not built — recon only, but requirement 1 turns out to need no patch at all**: `ICQ.cpp` hardcodes `Login("login.icq.com", 5190, …)` — on the retronet's DNS-hijacked, no-default-route addressing that hostname resolves straight to the gateway, same mechanism win98se's own doc names for its literal-IP setting. Two small patches remain: (1) `OSCARManager::HandleSSI()`'s `BUDDY_RECORD` case never parses the buddy item's alias TLV `0x0131` (contacts would render as bare UINs) — same fix shape as `GROUP_RECORD`'s existing inner-TLV loop three cases above it; (2) **no auto-reconnect/auto-login code exists**, but both hook points are now traced to single functions (`ICQProtocol::Init()` for auto-login — credentials are already populated by `UpdateSettings()` before `Init()` runs; `OSCARManager`'s `AMAN_CLOSED_CONNECTION` handler for a `BMessageRunner`-based reconnect timer, structurally modeled on tru64's shipped `rn_reconnect_schedule` patch) — estimated ~70-95 lines total, the same order of magnitude as the tru64 Gaim patch actually shipped. R5 buildability: the one real blocker found (`protocols/Jamfile` gates OSCAR on OpenSSL) is avoidable — login uses plain XOR password-roasting, not OpenSSL; only an optional buddy-icon upload touches `libcrypto`. Still unattempted. BeAIM fixes SSI/host-port too but is an AIM screen-name client (untested numeric-UIN path) with the same reconnect gap; not pursued further now that IM Kit's requirement-1 gap is free | native Be-API GUI client is the requirement (`docs/lab/retronet/ICQ-STATION.md`); IM Kit is the front-runner with a scoped ~70-95-line patch plan, not a dead end. Full source-level evidence, requirement-by-requirement, the reconnect engineering estimate, and the ICBM protocol-generation rejection: `docs/lab/ASSETS-MANIFEST.md` §"`beos` GUI OSCAR client — sourcing recon, revised 2026-08-22". climm 0.6.4 (already in this table) remains the fallback console client if the station ships before the IM Kit patch lands, with the same "fails the desktop-app requirement" caveat solaris/tru64 both had before their GUI swaps. |
| Gaim 0.59.9 | util | [SourceForge `pidgin` OldFiles](https://sourceforge.net/projects/pidgin/files/OldFiles/gaim-0.59.9.tar.gz/download) — `gaim-0.59.9.tar.gz`, 2 126 466 bytes, sha256 `268b630bfab1096b1cff4e02c97ea6bb2bf22b3be387d3c222cfe0453c86dbd8`; GTK+1.2.8/glib1.2.8 from the HP "Open Source Software Collection for Tru64 UNIX v5.1" **Disc 5** (archive.org `compaqtru64unix51`, `AG-RHAYC-BS.iso`, 629 368 832 bytes, sha1 `e153fb36c595575ce5c3013e3c3610eec7c131bc`) | free (GPLv2) | build from source on the guest with the native **Compaq C V6.5-011** (no gcc needed): `CONFIG_SHELL=/bin/ksh CC=/usr/bin/cc /bin/ksh ./configure --prefix=/usr/local --disable-prpls --disable-plugins --disable-perl --disable-gnome --disable-pixbuf --disable-esd --disable-artsc --disable-nls --disable-screensaver && make && make install`. **Tru64 needs `-liconv`** (iconv lives in `/usr/shlib/libiconv.so`, and configure records no library for it); a command-line `LIBS=` override does **not** reach Tru64 make's recursive sub-make, so set it before `configure` or edit `src/Makefile`. Disc 5 is **RPM, not `setld`**, and everything installs under `/usr/local` — extract the `glib`/`gtk+`/`gettext` payloads on labhost and deliver a tarball rather than running a package manager on a vendor OS. `configure` takes ~100 min on the emulated Alpha. Apply `streamhost/stations/tru64/gaim-0.59.9-icq-ssi.patch` to the source first | **LIVE on `tru64` since 2026-08-22, replacing climm** — the last GTK+1.2 Gaim, so a real desktop app (buddy list + chat windows under CDE/`dtwm`) is reachable on Alpha without a GTK+2 stack that does not exist for this platform. Stock 0.59.9 **refuses SSI on ICQ accounts** (`if (!odata->icq)` gates both the feedbag request and its parse); the patch removes both gates — our gateway serves the feedbag to an ICQ UIN fine — reads the nickname TLV `0x0131` so contacts render by name instead of bare UINs, ports auto-reconnect into the core (0.59.9 ships it only as a dlopen'd plugin, and this build is `--disable-plugins`), and fixes a real 0.59.9 bug that put the buddy list in the top-left corner after every reconnect. As-built: `docs/lab/retronet/ICQ-STATION-tru64.md`. |
