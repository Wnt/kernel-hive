...
| **ICBM .71 (BeCQ)** — BeOS `beos` station, **LIVE 2026-08-23** (IM Kit and BeAIM measured, not shipped) | util | ICBM .71 beta binary (`icbm.8k.com`, recovered via Wayback: `web.archive.org/web/20060109024731/http://icbm.8k.com:80/ICBM.71.zip`) archived sha256 `c8902f40714ef439a8abf5d8c92982eb144bc10ee0a0fb30f4255e08b8dd2dd1`, 182 919 bytes; measured-but-not-shipped: [github.com/HaikuArchives/IMKit](https://github.com/HaikuArchives/IMKit) HEAD `9c80ad1` sha256 `4eb6f38c3417dc6cb99610bd02fd86b32013f938b574d31462e1bb2221bd34e0`, [github.com/HaikuArchives/BeAIM](https://github.com/HaikuArchives/BeAIM) sha256 `fb3f2a63dba29943207d124d44e27448814c1ea5abe2edbb876fc16fa6b0cc87` | free (ICBM: GPL-2.0, `COPYING-2.0` shipped beside the binary; IM Kit: BSD; BeAIM: freeware, author disclaimers only) | **nothing to build** — a prebuilt `ICBM.x86` for R4.5/R5. FTP it to `/boot/home/apps/ICBM/`, then **`mimeset -f`** (FTP carries no BFS attributes, so without it the app runs but never registers its signature). Configure entirely with `addattr` over the exec channel — there is no config file, the settings ARE the attributes of a zero-byte `BeCQ-preferences`: `uin` (type `int`), `password`, `autologin` (1/0 — `true` silently stores 0) **before the first launch**, then `incomingopen=1` so an incoming message opens its own chat window. Seed HiveBot as a People file at `contacts/10000` (`BECQ:uin`, `META:name`, `META:nickname`, `META:shownick`) so it shows by name. Start it from `icbm-watchdog.sh`, not directly | **LIVE on `beos` since 2026-08-23** — the fleet's only pre-OSCAR client, and the only station that can reach the gateway's legacy **UDP 4000** door at all (every other ICQ station is behind a TCP-only slirp `guestfwd`). Era-correct (2001), native Be contact list and chat windows, no compiler needed. The `NetPacket.h` `operator new`/`delete` defect its own Readme warns about is **not present in the shipped `.71` binary** — `nm -D` shows no `__nw__15BStandardPacket…` import, i.e. the author built it with the fix applied — so the missing `.72` source (all Wayback copies are 403 stubs) does not matter. What it genuinely lacks is **auto-reconnect**: it honours the server's `0x00F0` teardown and then stays offline for ever, and it sends no `CMD_LOGOUT`, so the station supplies a 10-second guest-side watchdog that relaunches it (autologin makes the relaunch the reconnect). IM Kit's OSCAR engine **does build on R5** (`OSCARManager.cpp` → a 412 KB object under gcc 2.9-beos-991026 with only a `be_prim.h`→`SupportDefs.h` shim and a declaration-only `openssl/md5.h` stub, confirming the OpenSSL gate is avoidable) but its contact list needs Haiku's Layout Kit, absent from R5, and R5 has no `jam`. Full as-built: `docs/lab/retronet/STATION-beos.md` §The ICQ client. |
