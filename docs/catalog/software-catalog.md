...
| IM Kit + ICBM/BeCQ (OSCAR & legacy ICQ) — BeOS `beos` station, **BOTH REJECTED 2026-08-23, station stays retronet-only** | util | [github.com/HaikuArchives/IMKit](https://github.com/HaikuArchives/IMKit) source, HEAD `9c80ad1`, archived sha256 `4eb6f38c3417dc6cb99610bd02fd86b32013f938b574d31462e1bb2221bd34e0`; ICBM .71 beta binary (`icbm.8k.com`, recovered via Wayback) archived sha256 `c8902f40714ef439a8abf5d8c92982eb144bc10ee0a0fb30f4255e08b8dd2dd1`; secondary [github.com/HaikuArchives/BeAIM](https://github.com/HaikuArchives/BeAIM) sha256 `fb3f2a63dba29943207d124d44e27448814c1ea5abe2edbb876fc16fa6b0cc87` | free (IM Kit: BSD; ICBM: GPL-2.0; BeAIM: freeware, author disclaimers only) | **Both candidates were installed on the live station and measured against the real gateway; neither shipped.** ICBM is era-correct (2001, native Be contact list, needs no compiler) and DID sign UIN `50000` on to the gateway over the pre-OSCAR **UDP 4000** door — a door only `beos` can reach, since every other ICQ station is behind a TCP-only slirp `guestfwd` — but it never opens a contact-list window, never processes an inbound message the gateway is proven (tcpdump) to have sent, and exits unattended within minutes; that is the `NetPacket.h` `operator new`/`delete` defect its own Readme warns about, and it is unfixable because no `.71` source survives (all `.72` Wayback copies are 403 stubs). IM Kit's OSCAR engine **does build on R5** — `OSCARManager.cpp` compiled to a 412 KB object under gcc 2.9-beos-991026 with only a `be_prim.h`→`SupportDefs.h` shim and a declaration-only `openssl/md5.h` stub, confirming the OpenSSL gate is avoidable — but its contact-list client needs **Haiku's Layout Kit**, absent from R5, and R5 has no `jam`. | a native Be-API GUI client is the requirement (`docs/lab/retronet/ICQ-STATION.md`) and a Terminal client is explicitly out of bounds, so `beos` ships network + NetPositive only. Full evidence, both rejections, and the two things that would revive either candidate: `docs/lab/retronet/STATION-beos.md` §The ICQ client. climm 0.6.4 (already in this table) remains rejected for the same desktop-app reason. |
