...
| IM Kit (OSCAR/`ICQ.cpp`) — BeOS `beos` station, **SOURCING BLOCKED** | util | [github.com/HaikuArchives/IMKit](https://github.com/HaikuArchives/IMKit) source, archived at sha256 `4eb6f38c3417dc6cb99610bd02fd86b32013f938b574d31462e1bb2221bd34e0`; secondary candidate [github.com/HaikuArchives/BeAIM](https://github.com/HaikuArchives/BeAIM) at sha256 `fb3f2a63dba29943207d124d44e27448814c1ea5abe2edbb876fc16fa6b0cc87` | free (IM Kit: BSD, "Copyright (c) 2004-2009 IM Kit Team"; BeAIM: freeware, author disclaimers only) | **not built — recon only.** IM Kit needs three source patches before it is usable: (1) `ICQ.cpp` hardcodes `Login("login.icq.com", 5190, …)`, no server/port setting exists in the preflet at all; (2) `OSCARManager::HandleSSI()` downloads the feedbag correctly but never parses the buddy record's alias TLV `0x0131`, so contacts would render as bare UINs; (3) **no auto-reconnect or auto-login code exists anywhere in the tree** (unlike tru64's Gaim, where it existed as a disabled plugin to relocate) — would need to be designed and written from scratch. Upstream's own README states R5 is no longer supported; buildability with R5's gcc 2.95 is unverified. BeAIM fixes (1) via `AIMHost`/`AIMPort` prefs but is an AIM screen-name client (untested with a numeric ICQ UIN) and has the same missing-reconnect gap | native Be-API GUI client is the requirement (`docs/lab/retronet/ICQ-STATION.md`); **no candidate clears the bar as shipped** — this is the honest outcome, not climm-style "small patch and ship." Full source-level evidence, requirement-by-requirement: `docs/lab/ASSETS-MANIFEST.md` §"`beos` GUI OSCAR client — sourcing recon, BLOCKED without further engineering". climm 0.6.4 (already in this table) remains the fallback console client if the station ships before this is resolved, with the same "fails the desktop-app requirement" caveat solaris/tru64 both had before their GUI swaps. |
