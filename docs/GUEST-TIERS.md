...
- **Tier 1 (44)** — `aix432 alpine android aros aux beos bootos chokanji
  ...
  nt351 nt4 os2warp os213 postmarketos qnx ravynos reactos redhat62 redstar2
  redstar3 rhapsody
...
rhapsody sunos414 win311` — so they carry an interpretation layer the other 33
do not. `os213` is deliberately NOT in that list despite being a 1990 16-bit
guest: under TCG it traps seconds after its boot banner, and KVM is what makes
it run at all (see [`lab/OS213-WAVE.md`](lab/OS213-WAVE.md)). Every foreign-architecture guest is necessarily in that list (PowerPC,
...
| `a1000` | 3 host-native | FS-UAE/host | `x11-xtest` | abs | — | off | 50 | — |
