...
| [lab/QWENIT.md](lab/QWENIT.md) | **Codeword `qwenit`: offloading heavy work to a cheap open-weight model.** `scripts/dev/qwen-task.sh` wraps headless OpenCode on `openrouter/qwen/qwen3.8-27b` (~$0.006 for a small read-and-answer task; the cost is dominated by 8–10k input tokens per *step*, not by output length). The three safety caps and what a trip preserves; the API key's location and why it is never in this repo; and the three facts that mislead — there is **no sandbox** (bash and writes run ungated, `--auto` is not required), token usage is reported per-step and must be summed, and `last.md` is extracted from the event stream after exit rather than written by a flag. This repo is public, so a qwenit brief carries the scrubbed-placeholder rule explicitly and tasks **do not push**. Also documents `scripts/dev/qwen-watch.py`, the irssi-style live view (channel per task, aggregate channel 0) whose title bar shows live spend against the cap. |
...
| [lab/win311-interrupts-disabled-freeze.md](lab/win311-interrupts-disabled-freeze.md) | **Root-caused, not fixed: win311 freezes because the guest ends up running with the CPU interrupt flag clear.** Sustained keyboard input leaves `IF` clear and never re-enabled, so timer and keyboard IRQs sit pending forever (`pic0 irr=03 isr=00 imr=88`, IF clear in 10/10 samples) while QEMU keeps executing — which is why the clock stops, every app freezes at once, and only `loadvm` recovers it. Reproducible in ~16s with the tools in `scripts/dev/input-wedge-repro/`; includes the Clock-beside-SkiFree passive probe that finally separated "input is dead" from "Windows is dead", and the table of eleven hypotheses this killed along the way. |
