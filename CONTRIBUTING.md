...
it reads the week's commits and writes the summary; nothing about the notes is
derived from commit-message scopes any more. The recipe, including the
authoring prompt and its limits, is
[`docs/lab/RELEASE-NOTES-PROMPT.md`](docs/lab/RELEASE-NOTES-PROMPT.md).

Weeks end **Sunday 09:00 Europe/Helsinki** (exclusive), and only closed,
written-up weeks are published — the in-progress week has no summary file, so
it appears nowhere (`render` warns if it finds one dated forward). Week
numbers must run contiguously from 0, so a missed Sunday is written up before
the archive renders again. Never hand-edit the three outputs: run `make release-notes` and commit what it
writes, and `make release-notes-check` verifies they match a fresh render.

That still leaves your commit subject doing real work: it is what the weekly
pass reads to decide whether something happened. Write a short statement of
what changed — `tru64: web browser applied to the live golden` — not "fix
stuff", and keep the `scope: subject` habit, which stays the easiest way to
skim a week of history.
