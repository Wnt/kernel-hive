...
| `PTY allocation failed` — `ssh lab` works for commands but no NEW interactive login does | `devpts` is gone from the host's `/dev/pts`; remount it: `ssh lab 'mount -t devpts devpts /dev/pts -o rw,nosuid,noexec,relatime,gid=5,mode=620,ptmxmode=000'`. Cause: the host's `/dev` is `shared:2` propagation, so a chroot that does `mount --rbind /dev $ROOT/dev` makes its `/dev/pts` a PEER of the real one and the teardown's `umount` propagates back out. Sessions already holding a pty keep working, so nothing looks wrong until someone logs in. Never mount a chroot's API filesystems by hand — use `scripts/lib/chroot-guard.sh` (`chroot-guard run-private bash` for ad-hoc work; `chroot_guard_mount_api`/`chroot_guard_umount_all` in a script), proven by `tests/chroot-guard-selftest.sh` |
