...
| `qemu-guestram-abswrite` | no hardware cursor to close a loop over — classic Mac OS composites the sprite in software — so the engine writes the target straight into the guest OS's own pointer globals in LOW MEMORY (`MTemp`/`RawMouse`, published with `CrsrNew := CrsrCouple`) and Mac OS's cursor VBL task moves the pointer; `Mouse` is the OS's own read-back and must never be written. An absolute WRITE, not a loop: nothing converges, nothing is measured | `macos753` |
