...
images:
  - src: /posters/perq/desktop.webp
    alt: The POS G.7 shell on a PERQ 1 — black text on a pale page, on a tall portrait screen, the arrow pointer resting beside the prompt
    caption: The PERQ Operating System at its `sys:User>Guest>` prompt, on a screen shaped like a sheet of paper.
---
## Origins
...
POS is a single-user system that assumes the screen is the point. Log in as
`guest` with no password and the machine settles at its own shell: the status
line reads `sys:User>Guest>  POS G.7  a-boot`, with a clock ticking beside it
in real time, and the `>` prompt waiting below. The arrow you see resting on
the page is POS's own mouse cursor, drawn straight onto the character console
by the Kriz tablet's absolute position — no separate graphical mode needed.
