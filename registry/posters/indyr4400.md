---
title: SGI Indy R4400 — IRIX 6.5
subtitle: 1993 · the other Indy
hero: /posters/indyr4400/desktop.webp
images:
  - src: /posters/indyr4400/desktop.webp
    alt: The IRIX 6.5 Indigo Magic Desktop under 4Dwm, with the Toolchest docked in the upper-left corner and the demos user's icon column down the right edge
    caption: The Indigo Magic Desktop of the demos session — the Toolchest at upper left with its Desktop, Selected, Internet, Find, System and Help menus, and down the right edge the icons that came with an Indy: the camera it shipped with as standard, Start Demos, buttonfly, and fsn, the 3D file system navigator.
---
## Origins
...
IRIX 6.5, first released in 1998, was the final major line and received maintenance updates until 2006. Its desktop is the Indigo Magic Desktop under 4Dwm, which SGI derived from Motif's window manager and then restyled thoroughly — the drop-shadowed icons, the engraved bevels and the teal-and-grey palette are SGI's own. The Toolchest docked in the upper-left corner is the environment's signature: a permanent strip of menus from which the whole system is reached.

## What you're looking at

The `demos` user is logged in to an Indigo Magic Desktop session. The Toolchest sits in its corner with the standard Desktop, Selected, Internet, Find, System and Help menus. Down the right edge is the icon column that came with the machine: the camera, Start Demos, buttonfly and buttonflyAudio, fsn, the demos home folder, the dumpster, and Welcome to SGI.

The mottled teal root pattern is not an artifact. It is X11's default root stipple, which SGI simply left in place — a two-colour dither that a 24-bit framebuffer renders as visible speckle, and which anyone who sat in front of one of these machines will recognise immediately.

This is the second SGI Indy in the hall, and it is here to be compared with the first. The two exhibits run the same IRIX 6.5.22 install on the same emulated hardware except for the processor — and they are drawn by two completely different emulators, written decades apart in different languages by different people, both reconstructing the same 1993 workstation from documentation and dumps. That they agree, pixel for pixel, on what an Indy's desktop looked like is the quiet point of showing them side by side.
