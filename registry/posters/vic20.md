---
title: Commodore VIC-20 — CBM BASIC V2
subtitle: 1980 · VIC-20 (CBM BASIC V2)
hero: /posters/vic20/desktop.webp
images:
  - src: /posters/vic20/desktop.webp
    alt: The VIC-20 power-on screen — blue text on white paper inside a cyan border
    caption: Twenty-two columns, 3583 bytes free, and a cursor. Everything else you had to type.
---
## Origins

The VIC-20 began as a chip nobody wanted to buy. MOS Technology, which Commodore owned, had designed a video interface chip — the VIC — to sell to other manufacturers building terminals and games. The orders did not come. Rather than write the part off, Commodore engineers put a computer around it, and Jack Tramiel, who had been saying for years that the industry should be building computers for the masses and not the classes, priced it accordingly.

What the chip could do set the shape of everything else. Twenty-two characters across, twenty-three rows down, sixteen colours, and an output designed to survive the aerial socket of an ordinary television. Around it went a MOS 6502 and five kilobytes of memory, of which BASIC announces 3583 bytes free — the rest is spoken for by the screen, the operating system's working storage, and the colour map.

It reached Japan in 1980 as the VIC-1001, the United States the following year, and Europe as the VC-20. It was sold in department stores and toy shops rather than through computer dealers, and it was advertised on television by William Shatner, who asked why you would buy just a video game.

## Significance

It was the first computer of any kind to sell a million units, and it did so in 1982, two years after launch. That number is the whole argument. Before the VIC-20 a home computer was a thing a hobbyist ordered; after it, a computer was a thing a family could be sold at the same counter as a toaster.

The five kilobytes are not a footnote to that success — they are the reason for it. Everything about the machine is arranged around the cheapest parts that would still make a computer, and the arrangement is visible to anyone who uses it. Screen memory sits at address 7680 and colour memory at 38400, and a program that pokes bytes into those addresses will draw on the display directly, faster than BASIC can print. Add a memory expansion cartridge and both addresses move, which is why so many type-in listings of the period carry a warning about which machine they were written for.

## What you're looking at

The power-on screen is the entire user interface: a banner, a count of free bytes, and `READY.` There is no desktop, no menu and no file listing, because there is nothing yet to list. Programs arrived on cartridge, on cassette through the Datasette, or through the keyboard, one line at a time, from a magazine.

There is no mouse and nowhere to attach one. The keyboard is the instrument — full-travel, unusually good for the price, and upper case only in this mode, with a set of graphics characters printed on the key fronts for drawing pictures out of type.

## Legacy

Commodore discontinued the VIC-20 in January 1985, by which time its own successor had eclipsed it: the Commodore 64, which kept the case, the keyboard and the ports, and replaced everything that mattered inside. Roughly two and a half million VIC-20s were sold.

Its influence is easiest to see in who used it. This was, for an enormous number of people, the first computer in the house — small enough to be bought on a whim, cheap enough to be given to a child, and open enough that the only thing to do with it, once the cartridge games were finished, was to learn to program it.
