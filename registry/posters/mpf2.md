...
Taiwan had built parts for the computer industry for years. This was among the first machines the island sold under its own name to the wider world. The company that made it renamed itself Acer in 1987.
...
The economy bought something in return. The same bitmap serves text and graphics alike, so the display can carry 40 columns of characters, a coarse 48 by 40 grid of blocks, or 280 by 192 pixels in six colors — black, white, blue, orange, purple, and yellow — without separate hardware for each.

## What you're looking at

The banner and the `>` prompt are the whole of the machine's user interface at startup. There is no menu, no desktop, and no file listing — only BASIC, ready for a line number or a direct command. Not every ROM was in English: some units shipped with a Chinese-localized BASIC built in, and text composed stroke by stroke through the Dragon symbol system. Programs arrived on audio cassette, and a floppy drive was available for those who could afford one.

There is no mouse, and no place to plug one in. The keyboard is the entire instrument: 48 keys, upper case only.

## Legacy

The Microprofessor II sold modestly and is remembered chiefly by collectors. In Britain, where the 1983 advertising ran, it cost about a third of an Apple II — the "64K" in the headline was doing the persuading. Its importance is positional rather than commercial: an early, self-branded product from a country that had until then supplied other people's factories. The company outgrew the experiment quickly. By the mid-1980s it was building plain IBM PC-compatible machines for the MS-DOS market — the command-line world on another tile in this hall — and it grew from there into one of the largest computer manufacturers in the world.
