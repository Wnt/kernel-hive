...
That silicon needed customers before there was a computer to put it in. In 1986 Acorn shipped the ARM Evaluation System: an ARM, 4 MB of memory and a supervisor ROM on a podule that hung off a BBC Micro's Tube, with a set of floppy discs, for around £4,500. It is the first ARM product ever sold.

## Significance

The ARM Evaluation System is not a computer. It has no operating system, no keyboard, no screen and no filing system of its own. It is a processor on the end of a cable, and the BBC Micro on the other end — the machine that a moment ago *was* the computer — is demoted to a terminal and a disc controller.
...
Acorn spun the processor out in 1990 as a joint venture with Apple and VLSI Technology, on the strength of Apple wanting an ARM for the Newton. The new company, ARM Ltd., started with twelve engineers in a converted barn in Cambridgeshire and a business model nobody in the industry rated: it would not make chips at all, it would license the design and let other people make them.
