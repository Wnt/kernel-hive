...
| `SH_AUTO_RESET_CMD` | — | unset (off) | Argv (whitespace-split, no shell) of the station's reset, run by the daemon when a trigger below fires; the fleet's one reset is `/bin/bash /data/vms/streamhost/serve/reset-tile.sh <station>`, the same script as the gallery's Restore button. Needs the idle pauser's session bookkeeping, so the station must have a freezer (QEMU, or `SH_IDLE_PAUSE_PIDFILE`); with `SH_IDLE_PAUSE_SECS=0` the pauser runs track-only. `nokia9300` is the first user. See `IDLE-PAUSE.md` § Auto-reset. | auto_reset |
| `SH_AUTO_RESET_AFTER_SESSION_SECS` | — | `0` (off) | Reset this long after the LAST session left — only if the guest saw input since the last reset. A session that returns inside the grace (a reload) cancels it. | auto_reset |
| `SH_AUTO_RESET_INPUT_IDLE_SECS` | — | `0` (off) | Reset after this long with no input record at all, sessions or not (a tab left open) — only if the guest saw input since the last reset. A live wake lease withholds both triggers. | auto_reset |
