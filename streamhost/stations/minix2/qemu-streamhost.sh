...
set -e
[ -f "/data/vms/streamhost/stations/minix2/qemu.pid" ] && kill "$(cat "/data/vms/streamhost/stations/minix2/qemu.pid")" 2>/dev/null || true
sleep 0.3
rm -f "/data/vms/streamhost/stations/minix2/qmp.sock" "/data/vms/streamhost/stations/minix2/qemu.pid"
# streamhost display fast-poll (pve-qemu 0047): dbus poll every SH_DBUS_UPDATE_MS ms (default 4).
export SH_DBUS_UPDATE_MS="${SH_DBUS_UPDATE_MS:-4}"
LOADVM=""
qemu-img snapshot -l /data/vms/streamhost/stations/minix2/disk.qcow2 2>/dev/null | grep -qw golden && LOADVM="-loadvm golden -S"
# shellcheck disable=SC2086 # $LOADVM must word-split into -loadvm golden (or vanish when unset/cold-boot)
nohup qemu-system-x86_64 \
  -name streamhost-minix2 \
  ...
  \
  -drive file=/data/vms/streamhost/stations/minix2/disk.qcow2,format=qcow2,if=none,id=hd0 \
  -device ide-hd,drive=hd0,bus=ide.0,unit=0,cyls=200,heads=16,secs=32 \
  -qmp unix:/data/vms/streamhost/stations/minix2/qmp.sock,server=on,wait=off \
  -pidfile /data/vms/streamhost/stations/minix2/qemu.pid \
  >"/data/vms/streamhost/stations/minix2/qemu.log" 2>&1 &
for i in $(seq 1 40); do
  [ -S "/data/vms/streamhost/stations/minix2/qmp.sock" ] && [ -f "/data/vms/streamhost/stations/minix2/qemu.pid" ] && break
  sleep 0.5
done
echo "tile minix2 qemu pid=$(cat /data/vms/streamhost/stations/minix2/qemu.pid 2>/dev/null) qmp=/data/vms/streamhost/stations/minix2/qmp.sock udp=54201"
