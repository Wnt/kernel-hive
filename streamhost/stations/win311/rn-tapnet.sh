#!/bin/bash
# rn-tapnet.sh — the win311 station's link onto the retronet bridge vmbr-rn.
#
# Self-contained, per the standing rule against coupling sibling stations: this
# is win311's OWN copy of the bridge-enslaved tap pattern (see
# streamhost/stations/os2warp/rn-tapnet.sh and win95/rn-tapnet.sh for the
# siblings it is modelled on, and docs/lab/retronet/WEB-STATION-win311.md for
# this station's as-built).
#
# The guest is Windows for Workgroups 3.11 running Microsoft TCP/IP-32 (MSTCP32)
# over the RTL8029 NDIS3 driver (PCIND$) — the very card QEMU's ne2k_pci
# emulates. The stack DHCPs for its address (vdhcp.386; SYSTEM.INI [RTL80290]
# IPAddress=0.0.0.0), so it must share L2 with the retronet gateway CT
# (10.99.0.2) for the broadcast DHCP exchange, wildcard DNS, and multi-connection
# TCP to the gateway's :80 corpus origin. slirp carried none of that usefully —
# under slirp the guest leased 10.0.2.15 and had nothing to browse.
#
# A bridged WfW 3.11 is real exposure (vserver.386 file sharing is enabled and
# NBT is bound), so containment is layered and does NOT depend on any one thing:
#
...
#   2. ROUTING. The DHCP reservation (52:54:00:52:4e:1b -> 10.99.0.27) withholds
#      option 3 (router), and the in-guest SYSTEM.INI DefaultGateway is cleared,
#      so the guest has NO default route (Lock 1). labhost's retronet-fw FORWARD
#      chain (Lock 2) drops any vmbr-rn traffic trying to route THROUGH the box.
#   3. FILTER. This chain (below), scoped to the guest's source IP, lets the
#      guest reach labhost ONLY as the ESTABLISHED reply side of a
#      labhost-initiated connection. Every NEW flow the guest starts toward
#      labhost — the gallery on 10.99.0.1:8443, sshd, anything bound to the
#      bridge address — is DROPPED.
#
# win311's pointer path does NOT run over this netdev: the warpd agent is on a
# COM1 unix-socket serial chardev (guest-agents/win311/agent.c). Nothing but the
# guest's own TCP/IP rides this tap, so the slirp->tap swap costs the pointer
# nothing.
#
# Intra-bridge traffic (guest -> CT 10.99.0.2 for DHCP, DNS and HTTP) is pure L2
# with bridge-nf-call-iptables=0, so it never touches these chains — the
# retronet reaching the retronet is the point.
#
# Idempotent, and called `up` from qemu-streamhost.sh on EVERY launch, so it
# survives a station relaunch and a host reboot without a separate systemd unit.
# The tap is PERSISTENT: not torn down when QEMU stops. `down` is for teardown
# by hand.
#
...
GUEST_IP="${RN_TAP_GUEST_IP:-10.99.0.27}"
IN_CHAIN="WIN311RN-IN"
...
# connection labhost opened. Every NEW flow the guest starts toward labhost is
# dropped. Rebuilt each call.
install_rules() {
  iptables -w "$IPT_WAIT" -N "$IN_CHAIN" 2>/dev/null || true
  iptables -w "$IPT_WAIT" -F "$IN_CHAIN"
  # replies to a labhost-initiated probe (labhost dialled the guest first).
  ...
  # Hook the guest's source IP into INPUT, ABOVE retronet-fw's RETRONET-IN so
  # the scoped DROP wins over its blanket `-d 10.99.0.1 -j RETURN`. Re-inserted
  # at 1 on every launch (post-boot), which keeps it above the boot-time
  # RETRONET-IN.
...
do_up() {
  ...
  ip link show "$BRIDGE" >/dev/null 2>&1 || die "bridge $BRIDGE is absent (the gateway provisioner owns it)"
  ...
  # No L3 address on the tap: it is a pure bridge port. labhost reaches the
  # guest via the bridge's own 10.99.0.1. IPv6 off on the port for good measure
  ...
  msg "up: $IF enslaved to $BRIDGE; guest $GUEST_IP contained (replies only toward labhost; NEW->labhost dropped)"
}
