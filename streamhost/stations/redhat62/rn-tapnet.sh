#!/bin/bash
# rn-tapnet.sh — the redhat62 station's link onto the retronet bridge vmbr-rn.
#
# A direct copy of streamhost/stations/win95/rn-tapnet.sh (the bridged-tap
# recipe of docs/lab/retronet/ICQ-STATION.md) with this station's names.
# Red Hat Linux 6.2 joins the retronet **web** and **ICQ** planes, and OSCAR
# cannot traverse SLIRP, so the ICQ/web NIC must be a real bridge port. See
# docs/lab/retronet/STATION-redhat62.md.
#
# TWO NICs, deliberately (the amix precedent):
#   eth0  ne2k_pci on SLIRP — keeps the loopback X forward 127.0.0.1:6081 ->
#         10.0.2.15:6000 alive, which is the ONLY path the x11warp pointer
#         backend has. It is not touched by this script.
#   eth1  ne2k_pci on THIS tap — 10.99.0.33/24 static, DNS 10.99.0.2, and
...
#   2. ROUTING. The guest has NO default route on either NIC (SLIRP's DHCP is
#      not run; eth1 is static with no gateway), so its own stack cannot form a
#      packet to anything off 10.99.0.0/24. labhost's `retronet-fw` FORWARD
#      chain drops vmbr-rn traffic that tries to route THROUGH the box anyway.
...
#      reply side of a labhost-initiated connection. Unlike win95 this station
#      has no warpnet pointer/exec channel over the bridge at all — nothing on
#      labhost dials the guest here — so in practice the chain is a pure DROP;
#      the ESTABLISHED RETURN is kept so the shape matches the fleet and a
#      future labhost-initiated channel does not need a rule change.
#      Without it the guest could open labhost's 0.0.0.0 listeners by dialling
#      the bridge address 10.99.0.1, which retronet-fw deliberately leaves
#      reachable, and no-default-route does not close that because 10.99.0.1 is
#      ON the guest's own subnet.
#
# Intra-bridge traffic (guest -> gateway CT 10.99.0.2, for ICQ + the web plane)
# is pure L2 with bridge-nf-call-iptables=0, so it never touches these chains
# and is always allowed — that is the retronet reaching the retronet.
#
# Idempotent, and called `up` from qemu-streamhost.sh on EVERY launch. The tap
# is PERSISTENT and survives QEMU stopping. `down` is for teardown by hand.
#
...
IF="${RN_TAP_IF:-redhat62rn0}"
BRIDGE="${RN_TAP_BRIDGE:-vmbr-rn}"
# The guest's address on vmbr-rn — configured STATICALLY in the guest (RH 6.2
# ifcfg-eth1: BOOTPROTO=none, no GATEWAY), with a matching DHCP reservation kept
# on the gateway as the plane's uniqueness ledger (WEB-PLANE-PLAN.md: reserve
# first, then configure by hand to the address you reserved). The guard chain is
# scoped to this IP, so the filter follows the guest, never the whole bridge.
GUEST_IP="${RN_TAP_GUEST_IP:-10.99.0.33}"
IN_CHAIN="REDHAT62RN-IN"
...
# connection labhost opened (the warpnet pointer :7777 + exec :7788 channels).
# Every NEW flow the guest starts toward labhost is dropped. Rebuilt from empty
# each call.
install_rules() {
  iptables -w "$IPT_WAIT" -N "$IN_CHAIN" 2>/dev/null || true
  iptables -w "$IPT_WAIT" -F "$IN_CHAIN"
  # pointer + exec replies (labhost dialled the guest; this is the return traffic).
...
do_up() {
  ...
  msg "up: $IF enslaved to $BRIDGE; guest $GUEST_IP contained (pointer/exec replies only toward labhost; NEW->labhost dropped)"
}
