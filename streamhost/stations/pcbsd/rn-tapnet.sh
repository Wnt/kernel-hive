#!/bin/bash
# rn-tapnet.sh — the pcbsd station's link onto the retronet bridge vmbr-rn.
#
# This is the BRIDGE-enslaved cousin of streamhost/stations/irix/tapnet.sh, and a
# direct sibling of streamhost/stations/os2warp/rn-tapnet.sh, which it was copied
# from. The irix tap is a host-only /30 that REFUSES to ever be a bridge port;
# this one is the opposite by design — the PC-BSD 1.5.1 guest (FreeBSD 6.3-RELEASE
# + KDE 3.5.8) must share L2 with the retronet gateway CT (10.99.0.2) so its
# FreeBSD stack gets a real DHCP exchange (broadcast UDP) on em1, working ICMP,
# and real multi-connection TCP to the gateway's :80 web origin and :5190 OSCAR
# service. slirp could carry none of that for a stack that has to DHCP for its own
# address. See docs/lab/retronet/STATION-pcbsd.md.
#
# The guest is FreeBSD 6.3-RELEASE with pf enabled by default and a KDE 3.5.8
# desktop (Konqueror 3.5.8 on the web plane, Kopete 0.12.7 on the ICQ plane). A
# bridged BSD is real exposure, so containment is layered and does NOT depend on
# any one thing:
#
...
#   2. ROUTING. The guest takes its em1 address by DHCP from a reservation that
#      withholds option 3 (router), so this NIC gives it NO default route (Lock 1)
#      and its own stack cannot form a packet to anything off 10.99.0.0/24 over
#      it. labhost's `retronet-fw` FORWARD chain (Lock 2, Stream B) drops any
#      vmbr-rn traffic that tries to route THROUGH the box regardless.
#      pcbsd ALSO carries a second, slirp NIC (em0) that exists only to carry the
#      x11warp pointer door; it is launched `restrict=on`, so it reaches neither
#      labhost nor the outside world and only the hostfwd'd :6000 comes IN.
...
#      guest starts toward labhost — the gallery on 10.99.0.1:8443, sshd, anything
#      bound to the bridge address — is DROPPED. Without this the guest could open
#      labhost's 0.0.0.0 listeners by dialling the bridge address, which
#      retronet-fw deliberately leaves reachable (`RETRONET-IN` returns
#      -d 10.99.0.1), and no-default-route does not close it because 10.99.0.1 is
#      ON the guest's own subnet.
#   4. GUEST-SIDE pf. /etc/pf.conf carries a scoped `pass in quick on em1 from
#      10.99.0.0/24` / `pass out quick on em1 to 10.99.0.0/24` pair. pf is ON by
#      default in PC-BSD, so without it the guest filters its own retronet.
#
# Note pcbsd's pointer path does NOT run over this tap: x11warp reaches the
# guest's X server through the em0 slirp hostfwd. Nothing but the guest's own
# retronet TCP/IP depends on this tap.
#
# Intra-bridge traffic (guest -> CT 10.99.0.2, for DHCP, DNS and HTTP) is pure L2
# with bridge-nf-call-iptables=0, so it never touches these chains and is always
...
IF="${RN_TAP_IF:-pcbsdrn0}"
BRIDGE="${RN_TAP_BRIDGE:-vmbr-rn}"
# The guest's DHCP-reserved address on vmbr-rn. The guard chain is scoped to it, so the
# filter follows the guest, never the whole bridge.
GUEST_IP="${RN_TAP_GUEST_IP:-10.99.0.29}"
IN_CHAIN="PCBSDRN-IN"
...
# connection labhost opened (diagnostics — pcbsd's exec/pointer path is the COM1
# serial chardev, not this netdev, so nothing production-critical rides here).
# Every NEW flow the guest starts toward labhost is dropped. Rebuilt each call.
install_rules() {
