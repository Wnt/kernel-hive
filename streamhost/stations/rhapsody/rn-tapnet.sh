#!/bin/bash
# rn-tapnet.sh — the rhapsody station's link onto the retronet bridge vmbr-rn.
#
# This is the BRIDGE-enslaved cousin of streamhost/stations/irix/tapnet.sh, and a
# direct sibling of streamhost/stations/os2warp/rn-tapnet.sh. The irix tap is a
# host-only /30 that REFUSES to ever be a bridge port; this one is the opposite by
# design — the Rhapsody 5.1 DR2 guest must share L2 with the retronet gateway CT
# (10.99.0.2) so its 4.4BSD/Mach TCP/IP stack gets working ARP, ICMP and real
# multi-connection TCP to the gateway's :80 web origin and :53 resolver. slirp
# could carry none of that honestly for a guest that is meant to be a peer on a
# subnet. See docs/lab/retronet/WEB-STATION-rhapsody.md.
#
# The guest is Apple Rhapsody 5.1 DR2 for Intel (1998) — an OPENSTEP/Mach
# userland with a full BSD network stack, `inetd`, and NetInfo. A bridged
# Mach guest with 1998-vintage listeners is real exposure, so containment is
# layered and does NOT depend on any one thing:
#
...
#   2. ROUTING. The guest's address is configured with NO default route at all
#      (Lock 1) — the DHCP reservation withholds option 3 (router) and the
#      in-guest config names no gateway — so its own stack cannot form a packet
#      to anything off 10.99.0.0/24. labhost's `retronet-fw` FORWARD chain
#      (Lock 2) drops any vmbr-rn traffic that tries to route THROUGH the box
#      regardless.
...
#
# Note rhapsody's exec AND pointer paths do NOT run over this netdev: `labctl
# exec` is a root getty on the COM1 unix-socket serial chardev, and the pointer
# is PS/2 through the daemon's abs->rel bridge. Nothing but the guest's own
# TCP/IP depends on this tap, which is why the slirp->tap swap was free here.
#
...
GUEST_IP="${RN_TAP_GUEST_IP:-10.99.0.22}"
IN_CHAIN="RHAPRN-IN"
...
# connection labhost opened (diagnostics — rhapsody's exec path is the COM1 serial
# getty and its pointer is PS/2, not this netdev, so nothing production-critical
# rides here).
