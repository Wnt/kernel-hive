#!/bin/bash
# rn-tapnet.sh — the hpuxvue station's link onto the retronet bridge vmbr-rn.
#
# This is the BRIDGE-enslaved cousin of streamhost/stations/irix/tapnet.sh, and a
# sibling of win98se/solaris/tru64/w2kalpha's identically shaped helpers. The irix
# tap is a host-only /30 that REFUSES to ever be a bridge port; this one is the
# opposite by design — the HP-UX guest must share L2 with the retronet gateway CT
# (10.99.0.2) so it gets working ICMP + UDP (DHCP) and real multi-connection TCP
# straight to the gateway's :80 corpus origin, which slirp user-mode could not
# carry. See docs/lab/retronet/WEB-STATION-hpuxvue.md.
#
# The guest is HP-UX 10.20 — a 1996 System V stack with rpcbind, sendmail and the
# usual r-services listening. A bridged commercial Unix of that vintage is real
# exposure, so containment is layered and does NOT depend on any one thing:
#
...
#   2. ROUTING. The guest's DHCP reservation withholds option 3 (router), so it
#      has NO default route and its own stack cannot form a packet to anything
#      off 10.99.0.0/24. labhost's `retronet-fw` FORWARD chain (Lock 2, Stream B)
#      drops any vmbr-rn traffic that tries to route THROUGH the box regardless.
...
#      (hpuxvue has NO exec channel today — nothing on labhost dials this guest —
#      so in practice the ESTABLISHED rule never matches. It is kept identical to
#      the sibling stations so that wiring a serial/telnet exec channel later is
#      a station change, not a firewall change.)
#
# Intra-bridge traffic (guest -> CT 10.99.0.2, for DHCP + DNS + HTTP) is pure L2
...
GUEST_IP="${RN_TAP_GUEST_IP:-10.99.0.20}"
IN_CHAIN="HPUXRN-IN"
