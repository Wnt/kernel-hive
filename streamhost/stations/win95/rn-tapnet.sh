#!/bin/bash
# rn-tapnet.sh — the win95 station's link onto the retronet bridge vmbr-rn.
#
# The BRIDGE-enslaved cousin of streamhost/stations/irix/tapnet.sh, and a direct
# copy of the win98se/nt4 recipe (docs/lab/retronet/ICQ-STATION.md). The irix tap
# is a host-only /30 that REFUSES to ever be a bridge port; this one is the
# opposite by design — the Windows 95 OSR2 guest must share L2 with the retronet
# gateway CT (10.99.0.2) so it gets DHCP, working UDP + ICMP and real
# multi-connection TCP to the gateway's web-plane doors (what slirp's single-
# connection guestfwd could not carry). See docs/lab/retronet/WEB-STATION-win95.md.
#
# win95 is a 1996-era Win9x TCP/IP stack. A bridged Win9x is real exposure, so
...
#      reply side of a labhost-initiated connection. That covers BOTH labhost->
#      guest channels this station uses over the bridge: the warpnet POINTER
#      agent (labhost daemon -> guest:7777) and the warpnet EXEC agent (labctl
#      -> guest:7788). Every NEW connection the guest starts toward labhost — the
#      gallery on 10.99.0.1:8443, sshd, anything bound to the bridge address — is
#      DROPPED. Without this the guest could open labhost's 0.0.0.0 listeners by
#      dialling the bridge address, which retronet-fw deliberately leaves
#      reachable (`RETRONET-IN` returns -d 10.99.0.1), and no-default-route does
#      not close it because 10.99.0.1 is ON the guest's own subnet.
#
...
IF="${RN_TAP_IF:-win95rn0}"
BRIDGE="${RN_TAP_BRIDGE:-vmbr-rn}"
# The guest's address on vmbr-rn — a DHCP RESERVATION (retronet-dhcp pins
# RN_WIN95_MAC -> this IP, with DNS but NO router option), so it is fixed even
# though the guest configures itself. The guard chain is scoped to it, so the
# filter follows the guest, never the whole bridge.
GUEST_IP="${RN_TAP_GUEST_IP:-10.99.0.13}"
IN_CHAIN="WIN95RN-IN"
