...
#     tile comes up already on the GEOS deskTop (no C64 boot, no 1541 load, no keys).
...
set -e
BASE=/data/vms/streamhost/stations/c64
...
echo "tile c64 qemu pid=$(cat $BASE/qemu.pid 2>/dev/null) qmp=$BASE/qmp.sock udp=54114 ssh=127.0.0.1:5814 loadvm='${LOADVM:-<none: cold boot>}'"
