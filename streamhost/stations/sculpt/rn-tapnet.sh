#!/bin/bash
# rn-tapnet.sh — the sculpt station's link onto the retronet bridge vmbr-rn.
#
...
#      box regardless.
#   3. FILTER. This station's own fail-closed SCULPTRN-IN (below) lets the guest
...
#
# Station: sculpt (dhcp addressing). See docs/lab/retronet/STATION-sculpt.md.
set -u

IF="${RN_TAP_IF:-sculptrn0}"
...
GUEST_IP="${RN_TAP_GUEST_IP:-10.99.0.38}"
IN_CHAIN="${RN_TAP_IN_CHAIN:-SCULPTRN-IN}"
...
GUEST_MAC="${RN_TAP_GUEST_MAC:-$(sed -n 's/^[[:space:]]*RN_SCULPT_MAC=//p' /data/kernel-hive/registry/local.env 2>/dev/null | tail -1 | tr -d '\042\047')}"
GUEST_MAC="${GUEST_MAC:-02:00:00:00:00:26}"
