#!/bin/bash
# rn-tapnet.sh — the nt4 station's link onto the retronet bridge vmbr-rn.
#
# The BRIDGE-enslaved cousin of streamhost/stations/irix/tapnet.sh, and a direct
# copy of the win98se recipe (docs/lab/retronet/ICQ-STATION.md). The irix tap is
# a host-only /30 that REFUSES to ever be a bridge port; this one is the opposite
# by design — the Windows NT 4.0 guest must share L2 with the retronet gateway CT
# (10.99.0.2) so its ICQ 2000b client gets working UDP + ICMP and real
# multi-connection TCP straight to the OSCAR server (what slirp's single-
# connection guestfwd could not carry).
#
# The guest is Windows NT 4.0 SP6a — a 1999-era TCP/IP stack with the SMB/NetBIOS
# (netbt) listeners bound to this same adapter. A bridged NT4 is real exposure, so
...
GUEST_IP="${RN_TAP_GUEST_IP:-10.99.0.12}"
IN_CHAIN="NT4RN-IN"
