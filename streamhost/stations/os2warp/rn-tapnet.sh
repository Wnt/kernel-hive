#!/bin/bash
# rn-tapnet.sh — the os2warp station's link onto the retronet bridge vmbr-rn.
#
# This is the BRIDGE-enslaved cousin of streamhost/stations/irix/tapnet.sh, and a
# direct sibling of streamhost/stations/win98se/rn-tapnet.sh. The irix tap is a
# host-only /30 that REFUSES to ever be a bridge port; this one is the opposite by
# design — the OS/2 Warp 4.52 guest must share L2 with the retronet gateway CT
# (10.99.0.2) so its IBM TCP/IP stack gets a real DHCP exchange (broadcast UDP),
# working ICMP, and real multi-connection TCP to the gateway's :80 web origin.
# slirp could carry none of that for a stack that has to DHCP for its own address.
# See docs/lab/retronet/WEB-STATION-os2warp.md.
#
# The guest is IBM OS/2 Warp 4.52 (MCP2, kernel 14.089) running MPTS/NDIS + IBM
# TCP/IP 4.3 — a 2001-vintage stack with its own inetd-class listeners. A bridged
# OS/2 is real exposure, so containment is layered and does NOT depend on any one
# thing:
#
...
#   2. ROUTING. The guest takes its address by DHCP from a reservation that
#      withholds option 3 (router), so it has NO default route (Lock 1) and its
...
#
# Note os2warp's exec/pointer path does NOT run over this netdev: the warpd agent
# is on a COM1 unix-socket serial chardev. Nothing but the guest's own TCP/IP
# depends on this tap, which is why the slirp->tap swap was free here.
#
...
GUEST_IP="${RN_TAP_GUEST_IP:-10.99.0.19}"
IN_CHAIN="OS2RN-IN"
...
# connection labhost opened (diagnostics — os2warp's exec/pointer path is the COM1
