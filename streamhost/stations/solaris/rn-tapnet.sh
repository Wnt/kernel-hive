#!/bin/bash
# rn-tapnet.sh — the solaris station's link onto the retronet bridge vmbr-rn.
#
# The Solaris sibling of streamhost/stations/win98se/rn-tapnet.sh. The Win98
# station proved the host-side pattern (persistent tap enslaved to vmbr-rn +
# a fail-closed per-guest containment chain); this is the same host-side wiring
# for the Solaris 10 CDE guest, whose climm (OSCAR) ICQ client needs the same
# real L2 to the gateway CT (10.99.0.2) that slirp could not carry: working
# UDP + ICMP and real multi-connection TCP straight to the OSCAR server. See
# docs/lab/retronet/ICQ-STATION.md (win98se as-built) and ICQ-STATION-solaris.md.
#
...
#   2. ROUTING. The guest is configured with NO default route (Lock 1 — the
#      Solaris way: no /etc/defaultrouter, no `route add default`), so its own
...
#
# Unlike win98se, solaris' exec channel is labhost-INITIATED (the daemon /
# labctl dials the guest's warpd agent at 10.99.0.14:7777), so the guest never
# needs to start a flow toward labhost: the ESTABLISHED,RELATED return path is
# all the guest legitimately uses, and everything the guest itself initiates
# toward labhost is dropped. Intra-bridge traffic (guest -> CT 10.99.0.2, for
# ICQ + ping) is pure L2 with bridge-nf-call-iptables=0, so it never touches
...
GUEST_IP="${RN_TAP_GUEST_IP:-10.99.0.14}"
IN_CHAIN="SOLRN-IN"
