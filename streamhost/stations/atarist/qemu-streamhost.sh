...
# an INTERNAL 'golden' snapshot (full RAM+device state) of the running EmuTOS GEM desktop.
...
set -e
BASE=/data/vms/streamhost/stations/atarist
OVERLAY="$BASE/overlay.qcow2"
[ -f "$BASE/qemu.pid" ] && kill "$(cat "$BASE/qemu.pid")" 2>/dev/null || true
sleep 0.3
rm -f "$BASE/qmp.sock" "$BASE/qemu.pid"
# Boot straight into the golden GEM-desktop fixture if the snapshot is present.
LOADVM=""
...
echo "tile atarist qemu pid=$(cat $BASE/qemu.pid 2>/dev/null) qmp=$BASE/qmp.sock udp=54116 ssh=127.0.0.1:5816 loadvm='${LOADVM:-<none: cold boot>}'"
