#!/bin/bash
# rn-tapnet.sh — the win2000 station's link onto the retronet bridge vmbr-rn.
#
# The win2000 sibling of streamhost/stations/win98se/rn-tapnet.sh (which is the
# BRIDGE-enslaved cousin of irix/tapnet.sh). The Windows 2000 guest must share L2
# with the retronet gateway CT (10.99.0.2) so its ICQ 2000b client gets working
# UDP + ICMP and real multi-connection TCP straight to the OSCAR server (what
# slirp's single-connection guestfwd could not carry). See
# docs/lab/retronet/ICQ-STATION-win2000.md and ICQ-STATION.md (the win98se
# pathfinder this replicates).
#
# The guest is Windows 2000 — an NT 5.0 TCP/IP stack with SMB and other listeners.
# A bridged, contactable Windows box is real exposure, so containment is layered
# and does NOT depend on any one thing:
#
...
GUEST_IP="${RN_TAP_GUEST_IP:-10.99.0.11}"
IN_CHAIN="WIN2KRN-IN"
