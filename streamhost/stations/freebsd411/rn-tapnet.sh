#!/bin/bash
# rn-tapnet.sh — the freebsd411 station's link onto the retronet bridge vmbr-rn.
#
# The FreeBSD 4.11 sibling of streamhost/stations/beos/rn-tapnet.sh and
# streamhost/stations/win98se/rn-tapnet.sh, which proved the host-side pattern:
# a PERSISTENT tap enslaved to vmbr-rn plus a fail-closed per-guest containment
# chain, created from the launcher on every start. freebsd411 joined the
# retronet on 2026-09-03 so that Konqueror (KDE 3.3.2) can reach the gateway's
# :80 museum-corpus origin over real L2, and Kopete can reach its OSCAR door on
# 10.99.0.2:5190 — OSCAR cannot traverse the station's slirp NIC, which stays in
# place purely as the x11warp pointer path. See docs/lab/retronet/STATION-freebsd411.md,
# ICQ-STATION.md (the win98se pathfinder), WEB-PROXY.md and GATEWAY.md.
#
...
#   2. ROUTING. The guest has NO default route on this NIC (Lock 1): it is a
#      dhclient client and retronet-dhcp deliberately withholds option 3
#      (router), so the lease carries an IP + mask + DNS and nothing to route
#      through. labhost's `retronet-fw` FORWARD chain (Lock 2) drops any vmbr-rn
#      traffic that tries to route THROUGH the box regardless.
#   3. FILTER. This station's own fail-closed INPUT chain (below), scoped to
#      BOTH the guest's source IP and its source MAC, lets the guest reach
#      labhost ONLY as the ESTABLISHED reply side of a labhost-initiated flow.
#      Every NEW connection the guest starts toward labhost — the gallery on
#      10.99.0.1:8443, sshd, any 0.0.0.0 listener reachable via the bridge
#      address — is DROPPED. no-default-route does not close that on its own,
#      because 10.99.0.1 is ON the guest's own subnet.
#
# freebsd411 has NO exec channel (`exec_kind: null`): it is driven by QMP keys
# and the framebuffer, and the x11warp pointer rides the SEPARATE slirp NIC on
# loopback, never this bridge. So the guest legitimately initiates nothing at
# all toward labhost and the ESTABLISHED,RELATED return path is pure belt and
# braces. Intra-bridge traffic (guest -> CT 10.99.0.2 for DNS, the corpus web
# and OSCAR) is pure L2 with bridge-nf-call-iptables=0, so it never touches
# these chains and is always allowed — the retronet reaching the retronet.
#
...
GUEST_IP="${RN_TAP_GUEST_IP:-10.99.0.35}"
IN_CHAIN="FREEBSD411RN-IN"
...
# box-local in registry/local.env RN_FREEBSD411_MAC, exactly as the launcher reads it;
# the committed fallback is the scrubbed placeholder.
GUEST_MAC="${RN_TAP_GUEST_MAC:-$(sed -n 's/^[[:space:]]*RN_FREEBSD411_MAC=//p' /data/kernel-hive/registry/local.env 2>/dev/null | tail -1 | tr -d '\042\047')}"
GUEST_MAC="${GUEST_MAC:-02:00:00:00:00:23}"
...
install_rules() {
  ...
  # reply traffic for anything labhost itself dialled (freebsd411 has no exec
  # channel today; this keeps the chain identical to the fleet pattern).
  ...
  # still no WAN, but no longer matched by an -s 10.99.0.35 rule, i.e. free to
