#!/bin/bash
# rn-tapnet.sh — the winxp station's link onto the retronet bridge vmbr-rn.
#
...
# the opposite by design — the Windows XP guest must share L2 with the retronet
# gateway CT (10.99.0.2) so its period browser gets working ICMP + UDP (DHCP,
# DNS) and real multi-connection TCP straight to the corpus origin on :80 —
# which slirp's single-connection user-mode stack could not carry. See
# docs/lab/retronet/WEB-STATION-winxp.md.
#
# The guest is Windows XP Pro SP3 — a 2001 stack that is bridged with real
# siblings on vmbr-rn, so containment is layered and does NOT depend on any one
# thing:
#
...
#   2. ROUTING. The DHCP reservation withholds option 3 (router), so the guest
#      has NO default route (Lock 1) and its own stack cannot form a packet to
#      anything off 10.99.0.0/24. labhost's `retronet-fw` FORWARD chain (Lock 2,
#      Stream B) drops any vmbr-rn traffic that tries to route THROUGH the box
#      regardless.
...
#      -d 10.99.0.1), and no-default-route does not close it because 10.99.0.1
#      is ON the guest's own subnet.
#      (winxp has no exec channel today; the ESTABLISHED,RETURN rule is the
#      shared fleet shape and is what a future labhost-initiated exec channel
#      would ride. It grants the guest nothing it can initiate.)
#
# Intra-bridge traffic (guest -> CT 10.99.0.2, for DHCP/DNS/HTTP) is pure L2 with
...
GUEST_IP="${RN_TAP_GUEST_IP:-10.99.0.18}"
IN_CHAIN="WINXPRN-IN"
...
# connection labhost opened (a future exec channel; winxp has none today). Every
# NEW flow the guest starts toward labhost is dropped. Rebuilt from empty each call.
install_rules() {
  iptables -w "$IPT_WAIT" -N "$IN_CHAIN" 2>/dev/null || true
  iptables -w "$IPT_WAIT" -F "$IN_CHAIN"
  # replies to a labhost-initiated flow (labhost dialled the guest).
