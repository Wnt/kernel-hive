...
# MAME is fullscreen with aspect correction on the kiosk's 1024x768 X root,
# which is exactly the 4:3 of the television the ZX81 was designed to drive.
# Keyboard exhibit (no pointer; the ZX81's only other port was a cassette
# recorder). Kill only by pidfile.
#
...
# Boot straight into the golden ZX81 K-cursor fixture if the snapshot is present.
LOADVM=""
...
nohup qemu-system-x86_64 \
  ...
  -usb \
  -netdev user,id=n0,hostfwd=tcp:127.0.0.1:5831-:22 -device e1000,netdev=n0 \
  $LOADVM \
...
echo "tile zx81 qemu pid=$(cat $BASE/qemu.pid 2>/dev/null) qmp=$BASE/qmp.sock udp=54128 ssh=127.0.0.1:5831 loadvm='${LOADVM:-<none: cold boot>}'"
