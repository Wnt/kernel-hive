#!/bin/bash
# Launch tile 'kc854' (VMID 235) QEMU with the streamhost display wiring.
# BRIDGE tile: a captured Debian-12 kiosk running a purpose-built MAME 0.289
# `kc85_4` emulating the KC 85/4 (VEB Mikroelektronik "Wilhelm Pieck"
# Mühlhausen, 1988) at its CAOS 4.2 power-on screen — 320x256 @ 50.080411 Hz,
# drawn fullscreen with aspect correction on the bridge base's stock 1024x768 X
# root. Keyboard exhibit (no pointer). Kill only by pidfile.
#
...
# (full RAM+device state) of X (-nocursor) + MAME at the CAOS 4.2 menu.
...
#   * 768 MB is deliberate and measured: this guest runs one Z80 emulator under
...
# Boot straight into the golden CAOS 4.2 fixture if the snapshot is present.
LOADVM=""
...
nohup qemu-system-x86_64 \
  ...
  -usb \
  -netdev user,id=n0,hostfwd=tcp:127.0.0.1:5835-:22 -device e1000,netdev=n0 \
  $LOADVM \
...
echo "tile kc854 qemu pid=$(cat $BASE/qemu.pid 2>/dev/null) qmp=$BASE/qmp.sock udp=54132 ssh=127.0.0.1:5835 loadvm='${LOADVM:-<none: cold boot>}'"
