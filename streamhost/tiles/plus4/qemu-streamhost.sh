...
# BRIDGE tile: a captured Debian-12 kiosk running VICE `xplus4` emulating a PAL
# Commodore Plus/4 (1984), curated into the 3-plus-1 office suite that lives in
# the machine's ROM and resting in its SPREADSHEET — a yellow ruled grid on
# black, drawn double-size (-TEDdsize) on an 800x600 X root (the smallest
# advertised mode that contains VICE's fixed SDL window). Keyboard exhibit (no
...
# (full RAM+device state) of X (-nocursor) + xplus4 inside the 3-plus-1 suite.
