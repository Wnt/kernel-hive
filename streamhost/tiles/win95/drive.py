#!/usr/bin/env python3
# QMP driver for the streamhost win95 golden-fixture work.
# Connects to a QMP unix socket and runs input / screendump / snapshot commands.
# Win95 tile is machine usb=off + PS/2-relative mouse (no usb-tablet), so the
# pointer is driven via the LEGACY RELATIVE path (human-monitor-command mouse_move,
# 1:1 no accel) exactly like the tinycore golden bake. Keyboard is QMP qcode events.
import socket, json, sys, time

SOCK = sys.argv[1]
ARGS = sys.argv[2:]

RES_W, RES_H = 640, 480  # std VGA win95
...
# --- relative pointer (PS/2): slam to origin then step to target ---
def slam_origin():
    for _ in range(10):
        hmc("mouse_move -300 -300"); time.sleep(0.01)

def abs_move(x, y):
    slam_origin()
    x=int(x); y=int(y); dx=0; dy=0
    while dx < x:
        step=min(80,x-dx); hmc("mouse_move %d 0"%step); dx+=step; time.sleep(0.008)
    while dy < y:
        step=min(80,y-dy); hmc("mouse_move 0 %d"%step); dy+=step; time.sleep(0.008)

def click(x, y, button="left"):
    abs_move(x, y); time.sleep(0.2)
    b = 1 if button=="left" else (2 if button=="right" else 4)
    hmc("mouse_button %d"%b); time.sleep(0.12); hmc("mouse_button 0")
