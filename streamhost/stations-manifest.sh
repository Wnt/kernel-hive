...
# stations-manifest.sh — registry-generated recipe for all 92 production
...
#     bash stations-manifest.sh                 # emits all 92 production tiles' files
...
#
# PARITY GATE: scripts/dev/verify-emit.sh (repo) emits all 92 production tiles into a
...
if [ "$EMITTED" -ne 92 ]; then
  echo "FATAL: emitted $EMITTED tiles but the production roster is 92 — the sweep was truncated." >&2
  exit 1
fi
echo "=== all 92 production tiles emitted ==="
