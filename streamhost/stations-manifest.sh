...
# stations-manifest.sh — registry-generated recipe for all 93 production
...
#     bash stations-manifest.sh                 # emits all 93 production tiles' files
...
#
# PARITY GATE: scripts/dev/verify-emit.sh (repo) emits all 93 production tiles into a
...
if [ "$EMITTED" -ne 93 ]; then
  echo "FATAL: emitted $EMITTED tiles but the production roster is 93 — the sweep was truncated." >&2
  exit 1
fi
echo "=== all 93 production tiles emitted ==="
