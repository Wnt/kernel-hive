...
# stations-manifest.sh — registry-generated recipe for all 89 production
...
#     bash stations-manifest.sh                 # emits all 89 production tiles' files
...
#
# PARITY GATE: scripts/dev/verify-emit.sh (repo) emits all 89 production tiles into a
...
if [ "$EMITTED" -ne 89 ]; then
  echo "FATAL: emitted $EMITTED tiles but the production roster is 89 — the sweep was truncated." >&2
  exit 1
fi
echo "=== all 89 production tiles emitted ==="
