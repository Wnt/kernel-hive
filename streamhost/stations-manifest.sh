...
# stations-manifest.sh — registry-generated recipe for all 97 production
...
#     bash stations-manifest.sh                 # emits all 97 production tiles' files
...
#
# PARITY GATE: scripts/dev/verify-emit.sh (repo) emits all 97 production tiles into a
...
if [ "$EMITTED" -ne 97 ]; then
  echo "FATAL: emitted $EMITTED tiles but the production roster is 97 — the sweep was truncated." >&2
  exit 1
fi
echo "=== all 97 production tiles emitted ==="
