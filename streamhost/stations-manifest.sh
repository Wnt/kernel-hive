...
# stations-manifest.sh — registry-generated recipe for all 88 production
...
#     bash stations-manifest.sh                 # emits all 88 production tiles' files
...
#
# PARITY GATE: scripts/dev/verify-emit.sh (repo) emits all 88 production tiles into a
...
if [ "$EMITTED" -ne 88 ]; then
  echo "FATAL: emitted $EMITTED tiles but the production roster is 88 — the sweep was truncated." >&2
  exit 1
fi
echo "=== all 88 production tiles emitted ==="
