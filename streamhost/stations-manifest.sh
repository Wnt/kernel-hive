...
# stations-manifest.sh — registry-generated recipe for all 86 production
...
#     bash stations-manifest.sh                 # emits all 86 production tiles' files
...
#
# PARITY GATE: scripts/dev/verify-emit.sh (repo) emits all 86 production tiles into a
...
if [ "$EMITTED" -ne 86 ]; then
  echo "FATAL: emitted $EMITTED tiles but the production roster is 86 — the sweep was truncated." >&2
  exit 1
fi
echo "=== all 86 production tiles emitted ==="
