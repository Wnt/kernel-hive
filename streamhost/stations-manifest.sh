...
# stations-manifest.sh — registry-generated recipe for all 82 production
...
#     bash stations-manifest.sh                 # emits all 82 production tiles' files
...
#
# PARITY GATE: scripts/dev/verify-emit.sh (repo) emits all 82 production tiles into a
...
if [ "$EMITTED" -ne 82 ]; then
  echo "FATAL: emitted $EMITTED tiles but the production roster is 82 — the sweep was truncated." >&2
  exit 1
fi
echo "=== all 82 production tiles emitted ==="
