...
# stations-manifest.sh — registry-generated recipe for all 76 production
...
#     bash stations-manifest.sh                 # emits all 76 production tiles' files
...
#
# PARITY GATE: scripts/dev/verify-emit.sh (repo) emits all 76 production tiles into a
...
if [ "$EMITTED" -ne 76 ]; then
  echo "FATAL: emitted $EMITTED tiles but the production roster is 76 — the sweep was truncated." >&2
  exit 1
fi
echo "=== all 76 production tiles emitted ==="
