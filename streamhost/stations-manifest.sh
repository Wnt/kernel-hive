...
# stations-manifest.sh — registry-generated recipe for all 75 production
...
#     bash stations-manifest.sh                 # emits all 75 production tiles' files
...
#
# PARITY GATE: scripts/dev/verify-emit.sh (repo) emits all 75 production tiles into a
...
if [ "$EMITTED" -ne 75 ]; then
  echo "FATAL: emitted $EMITTED tiles but the production roster is 75 — the sweep was truncated." >&2
  exit 1
fi
echo "=== all 75 production tiles emitted ==="
