...
# stations-manifest.sh — registry-generated recipe for all 80 production
...
#     bash stations-manifest.sh                 # emits all 80 production tiles' files
...
#
# PARITY GATE: scripts/dev/verify-emit.sh (repo) emits all 80 production tiles into a
...
if [ "$EMITTED" -ne 80 ]; then
  echo "FATAL: emitted $EMITTED tiles but the production roster is 80 — the sweep was truncated." >&2
  exit 1
fi
echo "=== all 80 production tiles emitted ==="
