...
# stations-manifest.sh — registry-generated recipe for all 94 production
...
#     bash stations-manifest.sh                 # emits all 94 production tiles' files
...
#
# PARITY GATE: scripts/dev/verify-emit.sh (repo) emits all 94 production tiles into a
...
if [ "$EMITTED" -ne 94 ]; then
  echo "FATAL: emitted $EMITTED tiles but the production roster is 94 — the sweep was truncated." >&2
  exit 1
fi
echo "=== all 94 production tiles emitted ==="
