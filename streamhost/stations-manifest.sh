...
# stations-manifest.sh — registry-generated recipe for all 96 production
...
#     bash stations-manifest.sh                 # emits all 96 production tiles' files
...
#
# PARITY GATE: scripts/dev/verify-emit.sh (repo) emits all 96 production tiles into a
...
if [ "$EMITTED" -ne 96 ]; then
  echo "FATAL: emitted $EMITTED tiles but the production roster is 96 — the sweep was truncated." >&2
  exit 1
fi
echo "=== all 96 production tiles emitted ==="
