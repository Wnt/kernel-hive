...
# stations-manifest.sh — registry-generated recipe for all 72 production
...
#     bash stations-manifest.sh                 # emits all 72 production tiles' files
...
#
# PARITY GATE: scripts/dev/verify-emit.sh (repo) emits all 72 production tiles into a
...
if [ "$EMITTED" -ne 72 ]; then
  echo "FATAL: emitted $EMITTED tiles but the production roster is 72 — the sweep was truncated." >&2
  exit 1
fi
echo "=== all 72 production tiles emitted ==="
