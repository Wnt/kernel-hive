...
# stations-manifest.sh — registry-generated recipe for all 79 production
...
#     bash stations-manifest.sh                 # emits all 79 production tiles' files
...
#
# PARITY GATE: scripts/dev/verify-emit.sh (repo) emits all 79 production tiles into a
...
if [ "$EMITTED" -ne 79 ]; then
  echo "FATAL: emitted $EMITTED tiles but the production roster is 79 — the sweep was truncated." >&2
  exit 1
fi
echo "=== all 79 production tiles emitted ==="
