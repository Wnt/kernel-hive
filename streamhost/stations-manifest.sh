...
# stations-manifest.sh — registry-generated recipe for all 116 production
...
#     bash stations-manifest.sh                 # emits all 116 production tiles' files
...
#
# PARITY GATE: scripts/dev/verify-emit.sh (repo) emits all 116 production tiles into a
...
# openbsd (VMID 177) — OpenBSD 7.9 amd64 (2026), fvwm from base Xenocara; usb-tablet absolute pointer.
emit openbsd \
  --tile openbsd --vmid 177 --udp 54177 --pointer abs --audio on --fps 30 \
  --launcher-file "$T/openbsd/qemu-streamhost.sh" --env-append-file \
  "$T/openbsd/station.env.fixture"
...
# redhat62 (VMID 181) — Red Hat Linux 6.2 'Zoot' (2000), GNOME 1.0 + Enlightenment; x11warp pointer.
emit redhat62 \
...
# suse64 (VMID 180) — SuSE Linux 6.4 i386 (2000), KDE 1.1.2 on XFree86 3.3.6 (cirrus).
#   VERBATIM launcher: /opt/qemu-beos binary, -loadvm golden on disk.qcow2 (only block device),
#   ne2k_pci SLIRP with the loopback X forward 6080->6000 for the x11warp pointer.
emit suse64 \
  --tile suse64 --vmid 180 --udp 54180 --pointer abs --input-backend x11warp \
  --cursor-scale 1.0 --cursor-off-x 0 --cursor-off-y 0 --audio off --fps 30 \
  --launcher-file "$T/suse64/qemu-streamhost.sh" --env-append-file \
  "$T/suse64/station.env.fixture"
...
# medley — host-native Interlisp Medley (maiko Lisp VM under a pinned Xvfb, x11 capture, XTEST input; relaunch reset from the release sysout).
emit medley \
...
#   e1000 on the retronet tap sculptrn0 (the rtl8139 never transmits), no audio.
emit sculpt \
...
# lisa (VMID 193) — host-native LisaEm 2.0 (Apple Lisa 2, 68000 5 MHz, 1 MB, ROM rev H, ProFile) -> Lisa Office System 3.1, the document-centric 1984 desktop, inside a pinned 720x498 Xvfb whose root is the aspect-corrected Lisa screen. Absolute pointer over XTEST (LisaEm follows the host X pointer 1:1); reset = cold boot from a fresh ProFile copy (~150 s).
emit lisa \
  --tile lisa --udp 54193 --x11 --x11-display :93 --capture x11 --pointer \
  abs --input-backend x11test --audio off --fps 25 --x11-runtime-file \
  "$T/lisa/x11-runtime.sh" --aux-file "$T/lisa/lisa-inner.sh" \
  --env-append-file "$T/lisa/station.env.fixture"
...
# magiccap (VMID 197) — General Magic's Magic Cap for Windows, pre-release build 327, run as a Windows application inside the win98se-class guest.
emit magiccap \
  --tile magiccap --vmid 197 --udp 54197 --pointer abs --abs-pace-ms 30 \
  --audio on --fps 30 --launcher-file "$T/magiccap/qemu-streamhost.sh" \
  --aux-file "$T/magiccap/golden-bake.sh" --aux-file "$T/magiccap/qmp.py" \
  --aux-file "$T/magiccap/shot.sh" --aux-file "$T/magiccap/sk.py" \
  --env-append-file "$T/magiccap/station.env.fixture"
...
# pristine RP06 the source build assembled.
emit its \
  --tile its --udp 54216 --x11 --x11-display :116 --capture x11 --pointer \
  none --input-backend x11test --audio off --fps 25 --x11-runtime-file \
  "$T/its/x11-runtime.sh" --aux-file "$T/its/shared-terminal-runtime.sh" \
  --aux-file "$T/its/nspawn-inner.sh" --env-append-file \
  "$T/its/station.env.fixture"

# macosx (VMID 218) — Mac OS X 10.3 Panther on a PowerPC G4 mac99 under TCG,
# on the standalone /opt/qemu-ppc fork build. True absolute pointer (usb-tablet).
emit macosx \
  --tile macosx --vmid 218 --udp 54218 --pointer abs --cursor-scale 1.0 \
  --cursor-off-x 0 --cursor-off-y 0 --audio off --fps 30 --launcher-file \
  "$T/macosx/qemu-streamhost.sh" --env-append-file \
  "$T/macosx/station.env.fixture"
...
# multics — Multics MR12.8 on an emulated DPS-8/M under DPS8M, contained:
# simulator + Xvfb + one xterm on FNP channel d.h000 inside systemd-nspawn;
# relaunch reset from a pristine RPV baked to `shutdown complete`.
emit multics \
...
emit riscos3 \
  --tile riscos3 --udp 54213 --x11 --x11-display :73 --capture shm --pointer \
  none --input-backend mamesock --audio off --fps 60 --x11-runtime-file \
  "$T/mame-native/x11-runtime.sh" --aux-file "$T/riscos3/riscos3.keymap" \
  --env-append-file "$T/riscos3/station.env.fixture"

# vax43bsd — 4.3BSD on an Open SIMH VAX-11/780, contained: simulator + Xvfb + one
# xterm on the DZ11 visitor line inside systemd-nspawn; relaunch reset from a
# pristine RA81 that was halted clean.
emit vax43bsd \
  --tile vax43bsd --udp 54215 --x11 --x11-display :115 --capture x11 \
  --pointer none --input-backend x11test --audio off --fps 25 \
  --x11-runtime-file "$T/vax43bsd/x11-runtime.sh" --aux-file \
  "$T/vax43bsd/shared-terminal-runtime.sh" --aux-file \
...
emit linux012 \
  ...
  "$T/linux012/station.env.fixture"
...
emit nokia9300rom \
  ...
  "$T/nokia9300/nokia9300-inner.sh" --aux-file "$T/nokia9300/rn-netns.sh" \
  --aux-file "$T/nokia9300/x11test.keysyms" --env-append-file \
  "$T/nokia9300rom/station.env.fixture"
...
if [ "$EMITTED" -ne 116 ]; then
  echo "FATAL: emitted $EMITTED tiles but the production roster is 116 — the sweep was truncated." >&2
  exit 1
fi
echo "=== all 116 production tiles emitted ==="
