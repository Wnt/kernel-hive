...
# stations-manifest.sh — registry-generated recipe for all 68 production
...
#     bash stations-manifest.sh                 # emits all 68 production tiles' files
...
#
# PARITY GATE: scripts/dev/verify-emit.sh (repo) emits all 68 production tiles into a
...
if [ "$EMITTED" -ne 68 ]; then
  echo "FATAL: emitted $EMITTED tiles but the production roster is 68 — the sweep was truncated." >&2
  exit 1
fi
echo "=== all 68 production tiles emitted ==="
