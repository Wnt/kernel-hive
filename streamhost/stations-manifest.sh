...
# stations-manifest.sh — registry-generated recipe for all 78 production
...
#     bash stations-manifest.sh                 # emits all 78 production tiles' files
...
#
# PARITY GATE: scripts/dev/verify-emit.sh (repo) emits all 78 production tiles into a
...
if [ "$EMITTED" -ne 78 ]; then
  echo "FATAL: emitted $EMITTED tiles but the production roster is 78 — the sweep was truncated." >&2
  exit 1
fi
echo "=== all 78 production tiles emitted ==="
