...
# stations-manifest.sh — registry-generated recipe for all 100 production
...
#     bash stations-manifest.sh                 # emits all 100 production tiles' files
...
#
# PARITY GATE: scripts/dev/verify-emit.sh (repo) emits all 100 production tiles into a
...
# minix2 (VMID 95) — TODO one line; scaffolded from freedos.
emit minix2 \
  --tile minix2 --vmid 201 --udp 54201 --pointer rel --audio on --fps 30 \
...
if [ "$EMITTED" -ne 100 ]; then
  echo "FATAL: emitted $EMITTED tiles but the production roster is 100 — the sweep was truncated." >&2
  exit 1
fi
echo "=== all 100 production tiles emitted ==="
