...
# stations-manifest.sh — registry-generated recipe for all 83 production
...
#     bash stations-manifest.sh                 # emits all 83 production tiles' files
...
#
# PARITY GATE: scripts/dev/verify-emit.sh (repo) emits all 83 production tiles into a
...
if [ "$EMITTED" -ne 83 ]; then
  echo "FATAL: emitted $EMITTED tiles but the production roster is 83 — the sweep was truncated." >&2
  exit 1
fi
echo "=== all 83 production tiles emitted ==="
