...
# stations-manifest.sh — registry-generated recipe for all 98 production
...
#     bash stations-manifest.sh                 # emits all 98 production tiles' files
...
#
# PARITY GATE: scripts/dev/verify-emit.sh (repo) emits all 98 production tiles into a
...
if [ "$EMITTED" -ne 98 ]; then
  echo "FATAL: emitted $EMITTED tiles but the production roster is 98 — the sweep was truncated." >&2
  exit 1
fi
echo "=== all 98 production tiles emitted ==="
