...
# stations-manifest.sh — registry-generated recipe for all 84 production
...
#     bash stations-manifest.sh                 # emits all 84 production tiles' files
...
#
# PARITY GATE: scripts/dev/verify-emit.sh (repo) emits all 84 production tiles into a
...
if [ "$EMITTED" -ne 84 ]; then
  echo "FATAL: emitted $EMITTED tiles but the production roster is 84 — the sweep was truncated." >&2
  exit 1
fi
echo "=== all 84 production tiles emitted ==="
