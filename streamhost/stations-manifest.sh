...
# stations-manifest.sh — registry-generated recipe for all 95 production
...
#     bash stations-manifest.sh                 # emits all 95 production tiles' files
...
#
# PARITY GATE: scripts/dev/verify-emit.sh (repo) emits all 95 production tiles into a
...
if [ "$EMITTED" -ne 95 ]; then
  echo "FATAL: emitted $EMITTED tiles but the production roster is 95 — the sweep was truncated." >&2
  exit 1
fi
echo "=== all 95 production tiles emitted ==="
