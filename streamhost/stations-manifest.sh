...
# stations-manifest.sh — registry-generated recipe for all 99 production
...
#     bash stations-manifest.sh                 # emits all 99 production tiles' files
...
#
# PARITY GATE: scripts/dev/verify-emit.sh (repo) emits all 99 production tiles into a
...
if [ "$EMITTED" -ne 99 ]; then
  echo "FATAL: emitted $EMITTED tiles but the production roster is 99 — the sweep was truncated." >&2
  exit 1
fi
echo "=== all 99 production tiles emitted ==="
