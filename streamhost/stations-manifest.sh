...
# stations-manifest.sh — registry-generated recipe for all 74 production
...
#     bash stations-manifest.sh                 # emits all 74 production tiles' files
...
#
# PARITY GATE: scripts/dev/verify-emit.sh (repo) emits all 74 production tiles into a
...
if [ "$EMITTED" -ne 74 ]; then
  echo "FATAL: emitted $EMITTED tiles but the production roster is 74 — the sweep was truncated." >&2
  exit 1
fi
echo "=== all 74 production tiles emitted ==="
