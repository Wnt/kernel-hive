...
# stations-manifest.sh — registry-generated recipe for all 69 production
...
#     bash stations-manifest.sh                 # emits all 69 production tiles' files
...
#
# PARITY GATE: scripts/dev/verify-emit.sh (repo) emits all 69 production tiles into a
...
if [ "$EMITTED" -ne 69 ]; then
  echo "FATAL: emitted $EMITTED tiles but the production roster is 69 — the sweep was truncated." >&2
  exit 1
fi
echo "=== all 69 production tiles emitted ==="
