...
# stations-manifest.sh — registry-generated recipe for all 87 production
...
#     bash stations-manifest.sh                 # emits all 87 production tiles' files
...
#
# PARITY GATE: scripts/dev/verify-emit.sh (repo) emits all 87 production tiles into a
...
if [ "$EMITTED" -ne 87 ]; then
  echo "FATAL: emitted $EMITTED tiles but the production roster is 87 — the sweep was truncated." >&2
  exit 1
fi
echo "=== all 87 production tiles emitted ==="
