...
# stations-manifest.sh — registry-generated recipe for all 70 production
...
#     bash stations-manifest.sh                 # emits all 70 production tiles' files
...
#
# PARITY GATE: scripts/dev/verify-emit.sh (repo) emits all 70 production tiles into a
...
if [ "$EMITTED" -ne 70 ]; then
  echo "FATAL: emitted $EMITTED tiles but the production roster is 70 — the sweep was truncated." >&2
  exit 1
fi
echo "=== all 70 production tiles emitted ==="
