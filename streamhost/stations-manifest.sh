...
# stations-manifest.sh — registry-generated recipe for all 85 production
...
#     bash stations-manifest.sh                 # emits all 85 production tiles' files
...
#
# PARITY GATE: scripts/dev/verify-emit.sh (repo) emits all 85 production tiles into a
...
if [ "$EMITTED" -ne 85 ]; then
  echo "FATAL: emitted $EMITTED tiles but the production roster is 85 — the sweep was truncated." >&2
  exit 1
fi
echo "=== all 85 production tiles emitted ==="
