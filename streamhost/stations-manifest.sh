...
# stations-manifest.sh — registry-generated recipe for all 73 production
...
#     bash stations-manifest.sh                 # emits all 73 production tiles' files
...
#
# PARITY GATE: scripts/dev/verify-emit.sh (repo) emits all 73 production tiles into a
...
if [ "$EMITTED" -ne 73 ]; then
  echo "FATAL: emitted $EMITTED tiles but the production roster is 73 — the sweep was truncated." >&2
  exit 1
fi
echo "=== all 73 production tiles emitted ==="
