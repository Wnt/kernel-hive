...
# its (VMID 210) — TODO one line; scaffolded from medley.
emit its \
  --tile its --udp 54210 --x11 --x11-display :91 --capture x11 --pointer abs \
  --input-backend x11test --audio off --fps 25 --x11-runtime-file \
  "$T/its/x11-runtime.sh" --aux-file "$T/its/nspawn-inner.sh" \
  --env-append-file "$T/its/station.env.fixture"
