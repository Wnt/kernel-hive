...
# stations-manifest.sh — registry-generated recipe for all 77 production
...
#     bash stations-manifest.sh                 # emits all 77 production tiles' files
...
#
# PARITY GATE: scripts/dev/verify-emit.sh (repo) emits all 77 production tiles into a
...
if [ "$EMITTED" -ne 77 ]; then
  echo "FATAL: emitted $EMITTED tiles but the production roster is 77 — the sweep was truncated." >&2
  exit 1
fi
echo "=== all 77 production tiles emitted ==="
