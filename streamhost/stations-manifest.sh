...
# stations-manifest.sh — registry-generated recipe for all 91 production
...
#     bash stations-manifest.sh                 # emits all 91 production tiles' files
...
#
# PARITY GATE: scripts/dev/verify-emit.sh (repo) emits all 91 production tiles into a
...
# lisa (VMID 193) — TODO one line; scaffolded from amix.
emit lisa \
  --tile lisa --udp 54193 --x11 --x11-display :72 --capture x11 --pointer \
  abs --input-backend x11test --audio off --fps 25 --x11-runtime-file \
  "$T/lisa/x11-runtime.sh" --env-append-file "$T/lisa/station.env.fixture"
...
if [ "$EMITTED" -ne 91 ]; then
  echo "FATAL: emitted $EMITTED tiles but the production roster is 91 — the sweep was truncated." >&2
  exit 1
fi
echo "=== all 91 production tiles emitted ==="
