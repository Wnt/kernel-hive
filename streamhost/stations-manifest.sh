...
# stations-manifest.sh — registry-generated recipe for all 67 production
...
#     bash stations-manifest.sh                 # emits all 67 production tiles' files
...
#
# PARITY GATE: scripts/dev/verify-emit.sh (repo) emits all 67 production tiles into a
...
if [ "$EMITTED" -ne 67 ]; then
  echo "FATAL: emitted $EMITTED tiles but the production roster is 67 — the sweep was truncated." >&2
  exit 1
fi
echo "=== all 67 production tiles emitted ==="
