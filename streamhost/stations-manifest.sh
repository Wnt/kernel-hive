...
# stations-manifest.sh — registry-generated recipe for all 71 production
...
#     bash stations-manifest.sh                 # emits all 71 production tiles' files
...
#
# PARITY GATE: scripts/dev/verify-emit.sh (repo) emits all 71 production tiles into a
...
if [ "$EMITTED" -ne 71 ]; then
  echo "FATAL: emitted $EMITTED tiles but the production roster is 71 — the sweep was truncated." >&2
  exit 1
fi
echo "=== all 71 production tiles emitted ==="
