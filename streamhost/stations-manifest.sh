...
# stations-manifest.sh — registry-generated recipe for all 90 production
...
#     bash stations-manifest.sh                 # emits all 90 production tiles' files
...
#
# PARITY GATE: scripts/dev/verify-emit.sh (repo) emits all 90 production tiles into a
...
if [ "$EMITTED" -ne 90 ]; then
  echo "FATAL: emitted $EMITTED tiles but the production roster is 90 — the sweep was truncated." >&2
  exit 1
fi
echo "=== all 90 production tiles emitted ==="
