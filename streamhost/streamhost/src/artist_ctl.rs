//! `artistptr/1` pointer sink: the client half of the hpuxvue closed loop.
//!
...
//!
//! `hpuxvue` does not have to believe anything. HP-UX 10.20's X server drives
//! the B160L's Artist framebuffer HARDWARE cursor, so the guest writes the
//! pointer position into the Artist CURSOR_POS/CURSOR_CTRL registers on every
//! move — and the device model reads them. The closed loop therefore lives
//! INSIDE QEMU (`hw/display/artist.c`, on the kernel-hive hppa fork), at the
//! emulator's own rate and with direct register access, exactly as `aix432`
//! runs its engine over the Matrox DAC's CURPOSX/Y registers and `irix` runs
//! its MOVEA engine inside MAME over the Newport VC2 (`docs/IO-PATHS.md`).
//! This module is only the wire to it.
//!
//! Wire contract ("Closed-loop 1:1 pointer" in the Artist engine), IDENTICAL
//! in shape to `mgaptr/1` — only the banner token differs, so an Artist engine
//! can never be mistaken for a Matrox one:
//!
//! ```text
...
//! the guest's LASI PS/2 accumulator and neither will know where the cursor is.
//!
...
//!
//! **One divergence from `mga_ctl.rs`: a paused-guest ack grace.** hpuxvue
//! starts `-loadvm golden -S` (frozen until the first visitor) AND idle
//! auto-pauses (`SH_IDLE_PAUSE_SECS=60`, TCG burns a core for nobody). The
//! engine's move window is a VM-clock timer, so while the guest is stopped the
//! engine cannot ack anything — the socket stays open and the resync preamble
//! sits unacked. On aix432 that state is an engine fault and the sink
//! reconnects; here it is the NORMAL resting state of the station, and a hard
//! deadline turns it into a reconnect loop. See `ACK_PAUSED_GRACE`.
...
/// of `ptr-window-ms`) plus the edge pacer, hence the per-paced allowance.
const ACK_BASE: Duration = Duration::from_secs(5);
const ACK_PER_PACED: Duration = Duration::from_millis(200);

/// One grace period granted when the ack deadline first passes on a
/// connection, instead of reconnecting outright.
///
/// The constraint: this station's guest is routinely STOPPED — the launcher
/// starts it `-loadvm golden -S` and the daemon idle-pauses it after
/// `SH_IDLE_PAUSE_SECS=60`. The engine's move window is a VM-clock timer that
/// does not tick while the guest is stopped, so an unacked write is the
/// expected state of a healthy engine on a paused guest, not evidence of a
/// dead one. The sink has no clean way to OBSERVE pause state (QMP is owned
/// elsewhere in the daemon and we do not invent a side channel for this), so
/// the minimum honest response is to widen the deadline once: log, wait
/// `ACK_PAUSED_GRACE`, and only reconnect if the grace also expires. Any ack
/// proves the engine's timer is ticking again and clears the grace, so a
/// genuinely wedged engine is still bounded at BASE + GRACE per connection
/// while a guest that resumes mid-grace recovers with no reconnect at all.
const ACK_PAUSED_GRACE: Duration = Duration::from_secs(60);

/// `SH_ARTISTCTL_TRACE=on|1`: per-event wire tracing — ingress, every tx line
/// and every ack with its RTT. journald supplies the timestamps.
static TRACE: std::sync::OnceLock<bool> = std::sync::OnceLock::new();

fn trace_on() -> bool {
    *TRACE.get_or_init(|| {
        std::env::var("SH_ARTISTCTL_TRACE")
            .map(|v| v == "on" || v == "1")
            .unwrap_or(false)
    })
}

/// `SH_ARTISTCTL_SOCK`, defaulting to the station directory's `ptr.sock` — the
...
pub struct ArtistCtlSink {
    shared: Arc<Shared>,
}

impl ArtistCtlSink {
    pub fn new(path: String) -> Arc<Self> {
        eprintln!("[input-router] artistctl sink socket={path}");
        ...
        tokio::spawn(artistctl_task(path, shared.clone()));
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                eprintln!("[input-router] artistctl {}", log_shared.counters.line());
            }
        });
        Arc::new(Self { shared })
    }
...
impl RealtimeInputSink for ArtistCtlSink {
    fn try_pointer_abs(&self, event: PointerAbs) -> Result<AcceptedSeq, Reject> {
        ...
        if trace_on() {
            eprintln!(
                "[artistctl-trace] rx seq={} raw={},{} clamped={},{} btn={} ordered={}",
...
fn on_reply(line: &str, outstanding: &mut VecDeque<Sent>) {
    ...
    if let Some(i) = outstanding.iter().position(|s| s.seq == seq) {
        ...
        if trace_on() {
            eprintln!("[artistctl-trace] ack {seq} rtt_us={rtt_us}");
        }
        crate::input_telemetry::record_inject("artistctl", 1, rtt_us, None);
    }
    ...
    if kind == "ERR" {
        eprintln!("[artistctl] engine replied {line}");
    }
}
...
async fn send_cmd(
    ...
    *seq += 1;
    wr.write_all(format!("{} {}\n", *seq, cmd.line).as_bytes())
        .await?;
    if trace_on() {
        eprintln!("[artistctl-trace] tx {} {}", *seq, cmd.line);
    }
...
/// parse as `artistptr/1`, else the peer is not a compatible closed-loop engine
/// — which on this station means the QEMU binary predates it, and a silent
/// fallback would leave the pointer dead with no explanation. (The chardev
/// frontend sends the HELLO from QEMU's main loop, which keeps running while
/// the guest is stopped — only ACKS need the guest running, see
/// `ACK_PAUSED_GRACE`.)
async fn connect_artistptr(path: &str) -> std::io::Result<(EngineLines, OwnedWriteHalf)> {
    ...
    if !hello.starts_with("HELLO artistptr/1 ") {
...
async fn artistctl_task(path: String, shared: Arc<Shared>) {
    let mut backoff_ms = 50u64;
    let mut seq = 0u64;
    while !shared.closed.load(Ordering::Acquire) {
        shared.health.store(HEALTH_STARTING, Ordering::Release);
        match connect_artistptr(&path).await {
            Ok((mut lines, mut wr)) => {
                eprintln!("[artistctl] connected, HELLO verified {path}");
                backoff_ms = 50;
                run_connection(&shared, &mut lines, &mut wr, &mut seq).await;
            }
            Err(e) => {
                eprintln!("[artistctl] connect/HELLO {path} failed: {e}; retry {backoff_ms}ms");
            }
        }
...
async fn run_connection(
    ...
    for cmd in preamble {
        if let Err(e) = send_cmd(wr, seq, cmd, &mut outstanding).await {
            eprintln!("[artistctl] resync write failed: {e}; reconnecting");
            return;
        }
    }

    // Paused-guest grace, armed by the first ack-deadline expiry and cleared by
    // any engine line (any reply proves the engine's VM-clock timer is ticking,
    // i.e. the guest is running). See ACK_PAUSED_GRACE for the constraint this
    // exists for: on THIS station an unacked preamble on a freshly-restored,
    // still-frozen (-S) or idle-paused guest is the normal resting state, and
    // treating it as engine death turns rest into a reconnect loop.
    let mut pause_grace: Option<Instant> = None;
    ...
    loop {
        loop {
            ...
            if let Err(e) = send_cmd(wr, seq, cmd, &mut outstanding).await {
                eprintln!("[artistctl] write failed: {e}; reconnecting");
                return;
            }
        }
        ...
        let deadline = match (ack_deadline(&outstanding), pause_grace) {
            (Some(d), Some(g)) => Some(d.max(g)),
            (d, _) => d,
        };
        tokio::select! {
            ...
            line = lines.next_line() => match line {
                Ok(Some(line)) => {
                    pause_grace = None;
                    on_reply(&line, &mut outstanding);
                }
                Ok(None) => {
                    eprintln!("[artistctl] engine closed the socket; reconnecting");
                    return;
                }
                Err(e) => {
                    eprintln!("[artistctl] read failed: {e}; reconnecting");
                    return;
                }
            },
            _ = tokio::time::sleep_until(deadline.unwrap_or_else(Instant::now)),
                    if deadline.is_some() => {
                if pause_grace.is_none() {
                    // First expiry on this stretch of silence: assume a paused
                    // guest, not a dead engine, and wait the grace out. A guest
                    // resumed by an arriving visitor acks mid-grace and clears
                    // it; a genuinely wedged engine still reconnects, just
                    // ACK_PAUSED_GRACE later.
                    pause_grace = Some(Instant::now() + ACK_PAUSED_GRACE);
                    eprintln!(
                        "[artistctl] ack deadline passed ({} outstanding); guest may be \
                         paused (-S start / idle auto-pause), holding {}s before reconnect",
                        outstanding.len(),
                        ACK_PAUSED_GRACE.as_secs()
                    );
                } else {
                    eprintln!(
                        "[artistctl] ack timeout ({} outstanding, grace spent); reconnecting",
                        outstanding.len()
                    );
                    return;
                }
...
mod tests {
    ...
    const HELLO: &[u8] = b"HELLO artistptr/1 caps=movea,btn,sync,stat surf=1280x1024\n";

    fn bind(tag: &str) -> (std::path::PathBuf, UnixListener) {
        let dir = std::env::temp_dir().join(format!("artistctl-{tag}-{}", std::process::id()));
    ...
    async fn wait_healthy(sink: &ArtistCtlSink) {
        for _ in 0..200 {
    ...
    async fn restates_the_target_before_every_edge() {
        ...
        let sink = ArtistCtlSink::new(path.to_string_lossy().into_owned());
    ...
    async fn a_wrong_banner_is_refused() {
        ...
        // The SIBLING dialect is the likeliest wrong peer: same verbs, same
        // shape, wrong engine. Only the banner token tells them apart, so it
        // must be enough to refuse the connection.
        wr.write_all(b"HELLO mgaptr/1 caps=movea,btn,sync,stat surf=1024x768\n")
            .await
            .unwrap();
        // The connection is dropped rather than used: an engine that cannot
        // speak artistptr/1 must not be mistaken for one that can.
