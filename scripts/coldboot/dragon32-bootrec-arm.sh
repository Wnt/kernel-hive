...
# a per-tile file is where a reviewer can see one tile's capture parameters and
...
# and SKIP savevm/verify. Nothing is curated into this tile's fixture, so the
...
bootrec_scaffold_dragon32() {
  ...
  # `dragon`, not `mame`: the tile ships a MAME SUBTARGET=dragon build installed
