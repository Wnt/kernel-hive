...
| `lib/` | Shared infrastructure that is neither a tile, a stage, nor a toolchain build: `bridge-base.sh`, `graphical-bridge.sh`, `bbcmicro-type-qmp.py`, and the two one-line-cost executables `bridge-base-for`, `bridge-bake-golden`. | Each is used by MANY builders. `bridge-base.sh` builds the one frozen read-only base image that ~20 emulator-bridge tiles overlay — it *is* a `build-all.sh` manifest row, but it produces shared infrastructure, not an exhibit, and putting it in `tiles/` would make that directory lie. `graphical-bridge.sh` is a parameterized scaffold (`--tile/--install-script/--launch-script`) that any new graphical bridge instantiates; `irix/irix-bridge-install.sh` + `irix/irix-bridge-launch.sh` are its IRIX arguments. `bbcmicro-type-qmp.py` is the BBC-layout QMP typist shared by `tiles/bbcmicro.sh` and `tiles/armeval.sh`. The two extensionless executables are one-liners *at the call site* on purpose — the builders are dense scripts against the 600-line cap, so each carries its whole rationale in its own header: `bridge-base-for <tile>` resolves the suite's base qcow2, `bridge-bake-golden <qmp.sock> <overlay>` bakes and restore-proves the golden of an already-running tile. Separately, **every** ssh and scp a bridge builder runs passes `-o UserKnownHostsFile=/dev/null`: these are all loopback hostfwds into a guest the builder itself just created, so host-key verification protects nothing, while a rebuilt overlay's *changed* host key is a hard refusal that `StrictHostKeyChecking=no` does not cover. Add it to any new ssh/scp site you write here. |
