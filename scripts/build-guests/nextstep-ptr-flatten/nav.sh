...
TX=$1; TY=$2; ACT=${3:-none}; TAG=${4:-nav}
FLAT=${FLAT:-}
...
case "$ACT" in
  click) x "xdotool click 1" >/dev/null 2>&1; sleep 1.2; shot "$TAG-after";;
  dblclick) x "xdotool click 1; sleep 0.12; xdotool click 1" >/dev/null 2>&1; sleep 2.5; shot "$TAG-after";;
esac
