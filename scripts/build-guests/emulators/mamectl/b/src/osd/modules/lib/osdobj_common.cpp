...
#include "modules/netdev/netdev_module.h"
#include "modules/ctlsock/ctlsock.h"
...
void osd_common_t::init(running_machine &machine)
{
	...
	// mamectl (osgallery #45): unconditional module bring-up — the module
	// object, its ONE persistent timer and its notifiers exist in both the
	// enabled and disabled arms so the savestate signature is identical
	// across them; only the listener inside is MAME_CTL_SOCK-gated. See
	// src/osd/modules/ctlsock/ctlsock.cpp for the engraved covenants.
	ctlsock_init(machine);
}
