...
inline save_error save_manager::do_read(T check_length, U read_block, V start_header, W start_data)
{
	...
	auto const hdrcheck = validate_header(header, machine().system().name, sig);
	if (hdrcheck.first != STATERR_NONE)
	{
		// mamectl (osgallery #45): upstream computes the precise reason
		// ("Incompatible save file (signature X, expected Y)") and then
		// drops it, keeping only the error class. Say it: it is the one
		// observable that distinguishes a signature-orphaned golden from a
		// corrupt file, and the Stage-0 V4 signature checks key on it.
		osd_printf_error("state header check failed: %s\n", hdrcheck.second);
