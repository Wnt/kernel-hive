...
/***************************************************************************
    ...
    streamhost/stations/irix/irixagent.lua; that file stays in the repo as
    ...
    Env config (all optional except the gate):
      ...
      MAME_CTL_KEY_MOD_SEP   EMULATED ms a non-modifier PRESS must
                             wait after the last applied modifier
                             edge. For guests that do NOT scan the
                             matrix with the main CPU but hand it to
                             a slow keyboard controller with its own
                             sampling window (the Sinclair QL's 8049
                             IPC): a Shift that goes down in the SAME
                             sample as its character makes that
                             character vanish, at ANY typing pace,
                             because the browser sends the two edges
                             milliseconds apart and the drain applies
                             both in one pass. Measured on the QL rig:
                             0 ms lead drops the shifted character,
                             one guest scan (20 ms) of lead does not.
                                                                   [0]
...
public:
	...
	void init()
	{
		...
		m_key_mod_sep = double(env_long("MAME_CTL_KEY_MOD_SEP", 0)) / 1e3;
...
private:
	...
	void drain_keys()
	{
		...
		for (auto it = m_kq.begin(); it != m_kq.end(); )
		{
			...
			// A MODIFIER EDGE AND THE PRESS THAT FOLLOWS IT MUST NOT LAND IN
			// THE SAME GUEST SAMPLE (MAME_CTL_KEY_MOD_SEP, off by default).
			// The QL hands its matrix to an 8049 IPC, not to the 68008: the
			// controller samples on its own clock and reports the key with the
			// modifier level it saw, so a Shift whose DOWN edge arrives in the
			// same sample as its character makes that character VANISH — not
			// arrive unshifted, vanish. The browser sends those two edges ~5 ms
			// apart, well inside one drain pass, so the guest saw them together
			// at EVERY typing pace, which is why `+` dropped from `print 12+34`
			// at 220 ms spacing while every unshifted character survived.
			// Time only: nothing is reordered, so rules 1-3 above still hold,
			// and the gate clears by the passage of time like every other.
			if (m_key_mod_sep > 0.0 && !mod_key && e.val == 1
					&& now < m_mod_edge_at + m_key_mod_sep)
			{
				bar_press = true;
				blocked.insert(e.field);
				++it;
	...
	double m_key_mod_sep = 0.0;                   // MAME_CTL_KEY_MOD_SEP (emu s)
	double m_mod_edge_at = -1e9;                  // last applied modifier edge (emu s)
