#!/usr/bin/env python3
"""mctl-probe: minimal mamectl/1 seq/ack line client (issue #45 Stage 1).

The smallest thing that can talk to the mamectl OSD module's unix control
socket (`MAME_CTL_SOCK`, convention `<tile-dir>/ctl.sock`): connect, parse the
HELLO banner, send seq-stamped verb lines, await the matching OK/ERR ack, and
surface async EV lines. The Stage-1 rigs (goldtrace-record.py, ab-latency.py)
import `MctlClient` from this file; in Stage 3 it seeds labctl's mctl.py.
stdlib only, by design.

Protocol (mamectl/1, design section 3):
...
Timeout facts a caller must know (Stage-0 verdict, binding):
  ...
  * SAVEST acks on COMPLETION of a measured ~12 s stop-the-world
    immediate_save; probe it with --timeout >= 60.
...
like `MOVE -40 -40` are never mistaken for probe options; the split is done by
hand because argparse.REMAINDER swallows options placed after a positional):
    mctl-probe.py SOCK --hello
    mctl-probe.py SOCK [--timeout S] [--wait-ev NAME] VERB [ARG...]
    mctl-probe.py SOCK --stdin [--timeout S]
...
def split_cli(argv):
    """(head, verb): head = SOCK + probe options, verb = the verb line tokens.
...
def main():
    ap = argparse.ArgumentParser(description="minimal mamectl/1 probe (see module docstring)")
    ap.add_argument("sock", help="unix socket path (<tile-dir>/ctl.sock)")
    ap.add_argument("--timeout", type=float, default=10.0, help="per-ack timeout, s (SAVEST needs >= 60)")
    ...
    try:
        ...
        print(f"mctl-probe: {e}", file=sys.stderr)
        return 125
