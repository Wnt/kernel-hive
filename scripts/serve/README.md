...
| `tiles.json` | **Not in the repo** — the signaling registry (osId → udpPort + cert-hash file) is rendered from `registry/tiles/`: `python3 scripts/tiles-registry.py emit tiles.json`. The live copy at `/data/vms/streamhost/serve/tiles.json` is what the server reads; `serve-https-spa.sh manifests` renders and publishes it. |
