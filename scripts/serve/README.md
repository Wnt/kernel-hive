...
| `check-stream-tickets.py` | Fleet check: for every tile, does the ticket the gateway mints actually verify against that daemon's `SH_TILE`? Catches the id-divergence class of bug (`solaris`/`solariscde`) that otherwise only shows up as an exhibit freezing on reconnect. |
...
| `tiles.json` | Generated **reference** copy of the signaling registry (osId → udpPort + cert-hash file). Its row count comes from `registry/tiles/`; run `python3 scripts/tiles-registry.py count` for the current fleet total (currently 30 streamhost production tiles). Authoritative live copy lives at `/data/vms/streamhost/serve/tiles.json` on the host. |
| `webroot/gallery-manifest.json` | Generated public SPA lineup (museum metadata + archetype/transport/order/signal reference; no credentials). Copy to the same path under `/data/vms/streamhost/serve/` so it is served as `/gallery-manifest.json`. |
...
| `golden-manifest.json` | Committed reference copy of the per-tile golden-fixture manifest (reset mode, snapshot name, expected fixture description) that `reset-tile.sh` and the e2e checks consult. |
