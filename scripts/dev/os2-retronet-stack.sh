...
#
#   c) WINDOW GEOMETRY -> effectively fullscreen. The [screen] rect below used
#      to be xleft=40/ybottom=30/width=944/height=700 on a 1024x768 desktop --
#      only a 40-80px sliver of margin on every side, so the WPS desktop icon
#      column (SciTech SNAP, Klondike, OS/2 Chess, Mahjongg, DOOM, OS/2 Window,
#      WebExplorer (LAN), Get Netscape Navigator, OS/2 System, Assistance
#      Center, Connections, Programs) sat almost entirely BEHIND the browser
#      window; only wrapped label text peeked out past the left edge. The
#      operator asked for "not full screen, a smaller window" so the desktop,
#      its icons and the WPS chrome stay visible around it. The rect below is
#      a real windowed size: right two-thirds of the desktop (660/1024 =
#      64%), left edge at x=320 (well clear of the ~100px icon column), top
#      edge at y=50 (below the WPS status tray), bottom margin 108px. WebExplorer
#      reads this rect once at startup, so the fix is applied by re-launching
#      the browser (close + reopen), not by a one-off in-guest drag -- see
