...
# kill any prior rig emulator via /proc/<pid>/environ MAME_CTL_SOCK match (own rig only)
...
T1=$(date +%s.%N)
echo "SOCK_READY_ELAPSED $(python3 -c "print($T1-$T0)")"
python3 "$CTLPY" "$RIG/ctl.sock" --wait-settle "$RIG/coldboot_menu.png" --timeout 35 --interval 0.3 --settle-s 3
T2=$(date +%s.%N)
echo "COLDBOOT_TOTAL_ELAPSED $(python3 -c "print($T2-$T0)")"
