...
def main() -> int:
    ...
    ambiguous: list[str] = []
    pinned: dict[int, str] = {}  # code -> the default token that matched it (tier 1)
    ...
    # A name shared by several fields of ONE port ("Unnamed Key" for every
    # PORT_CODE-without-PORT_NAME key: the Apollo keyboard's Backspace, Tab and
    # Return, 2026-09-09) is ambiguous on the wire, where the module resolves
    # by name and takes the first. Such rows carry the field's default token
    # too (`name#KEYCODE_ENTER`; mame-ctlsock-field-token.patch), which makes
    # the match exact. Unique names stay bare, so existing keymaps are unchanged.
    name_count: dict[tuple[str, str], int] = {}
    for port, field, _token in fields:
        name_count[(port, field)] = name_count.get((port, field), 0) + 1
    for code in sorted(entries):
        port, field = entries[code]
        spec = field
        if name_count.get((port, field), 0) > 1:
            tok = pinned.get(code, "")
            if tok:
                spec = f"{field}#{tok}"
            else:
                ambiguous.append(
                    f"  {code:#04x} -> {port} {field!r} shared by "
                    f"{name_count[(port, field)]} fields and no token to pin it"
                )
        out_lines.append(f"{code:#04x}\t{port}\t{spec}")
    text = "\n".join(out_lines) + "\n"
    if args.out:
