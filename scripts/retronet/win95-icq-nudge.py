#!/usr/bin/env python3
"""win95-icq-nudge — keep the win95 ICQ persona reconnecting after a wake.
...
spoofed TCP ACK as if from the gateway (`10.99.0.2:5190 -> 10.99.0.13:<port>`)
...
win95-icq-nudge.timer; a no-op unless the guest is running AND offline.
"""
...
GUEST = os.environ.get("RN_ICQ_GUEST", "10.99.0.13")
PERSONA = os.environ.get("RN_ICQ_PERSONA_UIN", "95000")
GOLDEN_ICQ_PORT = int(os.environ.get("RN_ICQ_GOLDEN_PORT", "1025"))
CT = os.environ.get("RN_ICQ_CT", "951")
QMP = os.environ.get("RN_ICQ_QMP", "/data/vms/streamhost/stations/win95/qmp.sock")
PORTFILE = os.environ.get("RN_ICQ_PORTFILE", "/run/win95-icq-port")
...
def guest_running() -> bool:
    """True only if win95's QEMU is up and NOT paused (else nudging is pointless)."""
    try:
