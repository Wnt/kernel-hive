...
#
# Every generated artifact (see generated() in scripts/stations-registry.py) must be
...
# there is no second copy to drift. `make station-registry-check` proves they still
# render.
...
#
# Authoritative check: `make station-registry-check` renders every output in memory
...
# Generated artifacts, kept in lockstep with generated() in scripts/stations-registry.py.
GENERATED_PATHS=(
...
echo "== generated-file drift: make station-registry-check (byte parity) =="
make station-registry-check

if [[ "${1:-}" == "--regen" ]]; then
  echo "== generated-file drift: regenerate + git diff --exit-code =="
  make station-registry-generate >/dev/null
  if ! git diff --exit-code -- "${GENERATED_PATHS[@]}"; then
    echo "DRIFT: generated artifacts changed after 'make station-registry-generate'." >&2
    echo "       Run 'make station-registry-generate' and commit the result." >&2
    exit 1
  fi
fi
