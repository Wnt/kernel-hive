...
| `scripts/lib/checkpoint-guard.sh` | `/usr/local/bin/checkpoint-guard` | **HARD checkpoint-recapture guard** — `checkpoint-guard recapture <station>` is the whole safe sequence as one crash-safe operation (byte-copy backup SHA256-verified with the guest STOPPED, capture under the `cpg-staging` label, restore-proven on the framebuffer, only then the old label retired), with `resume`/`rollback`/`status`/`prune`. Never hand-roll `delvm golden; savevm golden` — that window left win95 with no checkpoint at all on 2026-08-24. Covers QEMU vmstate stations; refuses es40/MAME savestate stations loudly. One tool in three halves, all deployed together or the guard refuses to run: `checkpoint-guard-proof.sh` (the framebuffer proof), `checkpoint-guard-disks.sh` (which qcow2 holds the checkpoint, including for a STOPPED station whose launcher builds the path from `$D`) and `cpg-mask.py` (`CPG_MASK` — DECLARED rectangles excluded from the stability comparison, for a scene like apple.com's 1998 animated ticker that never idles; cropped not blanked, so the rest of the frame keeps the same threshold). See `docs/lab/checkpoint-guard.md` |
