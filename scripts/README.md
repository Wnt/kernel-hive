...
`scripts/dev/box-deploy.sh` (plan) / `--apply` runs `box-repo.sh sync` to
...
**The pre-push gate no longer blocks on "box behind main"** — that is now a
note (`box-deploy.sh --apply` after you push). What it still hard-fails on is
live files that differ from the commit the box checkout is actually at (a
hand edit or a half-finished install); see the "box state" section of
