...
import { CATALOG_ENTRIES } from './museumCatalog';
...
describe('museum copy stays visitor-facing', () => {
  ...
  it.each(CATALOG_ENTRIES.map((m) => [m.id, m] as const))('%s: lineage is a heritage, not a paragraph', (_id, m) => {
  ...
  it.each(CATALOG_ENTRIES.map((m) => [m.id, m] as const))('%s: no rig detail in visitor-facing copy', (_id, m) => {
  ...
  it.each(CATALOG_ENTRIES.map((m) => [m.id, m] as const))('%s: has a blurb of its own', (_id, m) => {
    // catalog.ts falls back to `notes` when `blurb` is missing, which would
    // put emulator/host detail in front of visitors.
    expect((m.blurb ?? '').trim().length).toBeGreaterThan(0);
  });

  it.each(CATALOG_ENTRIES.map((m) => [m.id, m] as const))('%s: states memory in a unit it can express', (_id, m) => {
