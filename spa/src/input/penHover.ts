...
//  flood of pointermove events. On a rel-pointer tile each forwarded move becomes
//  a RelMotion, so an idle hovering pen DRIFTS the guest cursor across the screen.
//  Gate hover deterministically: drop it outright on rel tiles; on abs tiles keep
...
export interface PenHoverInput {
  pointerType: string;
  buttons: number;
  /** true on SH_POINTER=rel tiles (qnx/freedos/msdoswin1). */
  rel: boolean;
  nowMs: number;
  lastMs: number;
...
const HOVER_MIN_MS = 32; // ~30 Hz ceiling for abs-tile pen hover

/** True if this move should be forwarded to the guest. A pen hover on a rel tile
 *  is always dropped; on an abs tile it is throttled to `minIntervalMs`. Anything
